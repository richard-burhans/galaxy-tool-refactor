...
def test_change_is_frozen() -> None:
    change = Change(
        code="GTX002", sourceline=4, xpath="/tool", message="m", mutate=_noop
    )
    with pytest.raises(dataclasses.FrozenInstanceError):
        change.code = "GTX999"  # type: ignore[misc]
...
def test_change_equality_ignores_the_thunk() -> None:
    ...
    one = Change(code="GTX002", sourceline=4, xpath="/tool", message="m", mutate=_noop)
    two = Change(
        code="GTX002", sourceline=4, xpath="/tool", message="m", mutate=lambda: None
    )
    assert one == two
