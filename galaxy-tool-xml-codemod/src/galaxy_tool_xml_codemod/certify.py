...
tier-1 static policy ``galaxy_tool_xml.shell_oracle.quote_is_behavior_preserving`` — the
...
``galaxy-tool-xml[shell-oracle]`` extra is absent.
...
if TYPE_CHECKING:
    from galaxy_tool_xml.command_text import UnquotedVar
...
class EditCertifier(Protocol):
    ...
    def should_quote(
        ...
        *,
        ...
        ``galaxy_tool_xml.shell_oracle.quote_is_behavior_preserving`` so a certifier and
