...
def test_measure_command_quoting_kinds_splits_keep_vs_drop(tmp_path: Path) -> None:
    ...
    # Auto-quoted: ONLY the IUC-scope files (input + output <data>) — codemod §52.
    assert result.quoted_by_kind["input-file"] == 1
    assert result.quoted_by_kind["output-file"] == 1
    # Out of the IUC file scope -> NOT quoted (safe no-ops left alone):
    assert "select" not in result.quoted_by_kind
    assert "attr" not in result.quoted_by_kind
    assert "builtin" not in result.quoted_by_kind
    assert "text-param" not in result.quoted_by_kind
