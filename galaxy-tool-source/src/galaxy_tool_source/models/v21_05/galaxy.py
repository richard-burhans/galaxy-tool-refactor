...
class ParamOptions:
    ...
    * Using ``filter``s - various filters can be used to populate options,
    ...
    the dataset itself. ```xml &lt;param name="input" type="data"
    format="maf" label="MAF File"/&gt; &lt;param name="species"
    ...
    :ivar from_dataset: Determine options from the dataset given in the
        referred input parameter. If 'meta_file_key' is given, the
        options are determined from the data in the metadata file of the
        input.
    ...
    :ivar startswith: Ignore lines starting with the given character.
...
class Param(InputType):
    ...
    :ivar collection_type: Restrict the kind of collection that can be
        ...
        as a comma-separated list. Used only when the ``type`` attribute
    ...
    :ivar truevalue: The parameter value in the Cheetah template if the
        parameter is ``true`` or checked by the user. Used only when the
        ``type`` attribute value is ``boolean``.
    :ivar falsevalue: The parameter value in the Cheetah template if the
        parameter is ``false`` or not checked by the user. Used only
...
class Tool:
    ...
    default stdio error condition (before non-empty stderr). #### 18.01 -
    Use a separate home directory for each job. - Introduce
    `provided_metadata_style` with default `"default"` before `"legacy"`.
    #### 18.09 - References to other inputs need to be fully qualified by
    using `|`. - Do not allow provided but illegal default values. - Do not
    use Galaxy python environment for `manage_data` tools. #### 19.05 -
    Change default Python version from 2.7 to 3.5 #### 20.05 - json config
    files: - unselected optional `select` and `data_column` parameters get
    `None` instead of `"None"` - multiple `select` and `data_column`
    parameters are lists (before comma separated string) #### 20.09 - Exit
    immediately if a command exits with a non-zero status (`set -e`). -
    Assume sort order for collection elements. ### Examples A normal tool:
    ```xml &lt;tool id="seqtk_seq" name="Convert FASTQ to FASTA"
    version="1.0.0" profile="16.04" &gt; ``` A ``data_source`` tool
