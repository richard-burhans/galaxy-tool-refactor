...
``galaxy_tool_source.models`` (``v16_10`` … ``v26_0``). This module resolves an
...
def model_module(version: str) -> ModuleType:
    ...
    return importlib.import_module(
        "galaxy_tool_source.models." + version_to_module(version)
    )
...
def tool_class(version: str) -> type:
    ...
    if not isinstance(tool, type):
        raise TypeError(
            f"galaxy_tool_source.models.{version_to_module(version)} "
