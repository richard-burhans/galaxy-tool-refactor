...
class Command:
    ...
    an older Galaxy runtime). ``\\$GALAXY_MEMORY_MB`` | Amount of memory in
    ...
    not attempt to limit memory usage. See the [Planemo
    ...
    ``Exception:``, ``Error:``, or various messages related to being out of
    memory appear in the standard error or output. (The @bgruening
    recommendation). For newer tools with ``profile&gt;=16.04``, the
    default behavior is ``exit_code``. Legacy tools default to ``default``
    behavior described above (erroring if the tool produces any standard
    error output). See [pull request
    ...
    :ivar value:
    :ivar detect_errors:
    :ivar oom_exit_code: Only used if ``detect_errors="exit_code", tells
        Galaxy the specified exit code indicates an out of memory error.
        Galaxy instances may be configured to retry such jobs on
...
class ConfigFile:
    ...
    read.table("${s.input.file_name}") x${i} = s${i}[,${s.xcol}] y${i} =
    s${i}[,${s.ycol}] xrange = range(x${i}, xrange) yrange = range(y${i},
    yrange) #end for ## Open output PDF file pdf("${out_file1}") ## Dummy
    plot for axis / labels plot(NULL, type="n", xlim=xrange, ylim=yrange,
    main="${main}", xlab="${xlab}", ylab="${ylab}") ## Plot each series
    #for $i, $s in enumerate($series) #if $s.series_type['type'] == "line"
    lines(x${i}, y${i}, lty=${s.series_type.lty}, lwd=${s.series_type.lwd},
...
class VersionCommand:
    ...
    :ivar interpreter: *Deprecated*: Prefix the version command with an
        ...
        ``&lt;interpreter&gt;
        '$__tool_directory__/&lt;script_name&gt;'``.
    """
...
class ExitCode:
    ...
    ```xml &lt;stdio&gt; &lt;exit_code range="3:5" level="warning"
...
class Regex:
    ...
    description="Unknown error encountered" /&gt; &lt;!-- Catching
    fatal_oom allows the job runner to potentially resubmit to a resource
    with more memory if Galaxy is configured to do this. --&gt; &lt;regex
    match="out of memory" source="stdout" level="fatal_oom"
    description="Out of memory error occurred" /&gt; &lt;regex
...
class Validator:
    ...
    :ivar table_name: Tool data table name to check against if ``type``
        ...
        [tool data tables](https://galaxyproject.org/admin/tools/data-
        tables) and [data
        managers](https://galaxyproject.org/admin/tools/data-managers/)
...
class RequestParameterTranslation:
    ...
    ``tool_type`` attribute is ``data_source``). This tag set contains a
    set of [request_param](#tool-request-param-translation-request-param)
    elements.
    """
...
class Repeat(InputType):
    ...
    ``&lt;configfile&gt;`` tag set. ```xml #for $i, $s in
    enumerate($series) rank_of_series=$i input_path='${s.input}'
    x_colom=${s.xcol} y_colom=${s.ycol} #end for ``` ### Testing This is an
    example test case with multiple repeat elements for the example above.
    ```xml &lt;test&gt; &lt;repeat name="series"&gt; &lt;param name="input"
