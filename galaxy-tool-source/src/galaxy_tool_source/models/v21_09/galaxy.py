...
class AssertHasSize:
    """
    Asserts the output has specific size (in bytes), e.g. ``&lt;has_size
    value="10000" delta="100" /&gt;``.

    :ivar value: Desired size of the outpyt (in bytes)
    :ivar delta: Maximum allowed size difference (default is 0).
    """
...
class Validator:
    ...
    within a given range. - ``dataset_metadata_in_data_table``: Check if a
    metadata value is contained in a column of a data table. -
