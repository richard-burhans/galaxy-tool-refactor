...
class ChangeFormatWhen:
    """
    If the data type of the output dataset is the specified type, the data
    ...
    :ivar input: This attribute should be the name of the desired input
        parameter (e.g. ``input="out_format"`` above).
...
class Code:
    ...
    **Deprecated** do not use this unless absolutely necessary.
    ...
    The extensions described here can cause problems using your tool with
    certain components of Galaxy (like the workflow system). It is highly
    recommended to avoid these constructs unless absolutely necessary. This
    tag set provides detailed control of the way the tool is executed. This
    (optional) code can be deployed in a separate file in the same
    directory as the tool's config file. These hooks are being replaced by
    new tool config features and methods in the
    ~/lib/galaxy/tools/__init__.py code file. ### Examples #### Dynamic
    Options Use associated dynamic select lists where selecting an option
    in the first select list dynamically re-renders the options in the
    second select list. In this example, we are populating both dynamic
    select lists from metadata elements associated with a tool's single
    input dataset. The 2 metadata elements we're using look like this.
    ```python MetadataElement( name="field_names", default=[], desc="Field
    names", readonly=True, optional=True, visible=True, no_value=[] ) # The
    keys in the field_components map to the list of field_names in the
    above element # which ensures order for select list options that are
    built from it. MetadataElement( name="field_components", default={},
    desc="Field names and components", readonly=True, optional=True,
    visible=True, no_value={} ) ``` Our tool config includes a code file
    tag like this. ```xml &lt;code file="tool_form_utils.py" /&gt; ``` Here
    are the relevant input parameters in our tool config. The first
    parameter is the input dataset that includes the above metadata
    elements. ```xml &lt;param name="input" type="data"
    format="vtkascii,vtkbinary" label="Shape with uncolored surface
    field"&gt; &lt;validator type="expression" message="Shape must have an
    uncolored surface field."&gt;value is not None and
    len(value.metadata.field_names) &gt; 0&lt;/validator&gt; &lt;/param&gt;
    ``` The following parameter dynamically renders a select list
    consisting of the elements in the ``field_names`` metadata element
    associated with the selected input dataset. ```xml &lt;param
    name="field_name" type="select" label="Field name"
    refresh_on_change="True"&gt; &lt;options&gt; &lt;filter
    type="data_meta" ref="input" key="field_names"/&gt; &lt;validator
    type="no_options" message="The selected shape has no uncolored surface
    fields." /&gt; &lt;/options&gt; &lt;/param&gt; ``` The following
    ...
    field_component in enumerate( field_components ): options.append( (
    field_component, field_component, i == 0 ) ) return options ``` ####
    Parameter Validation This function is called before the tool is
    executed. If it raises any exceptions the tool execution will be
    aborted and the exception's value will be displayed in an error message
    box. Here is an example: ```python def validate(incoming): '''Validator
    for the plotting program''' bins = incoming.get("bins","") col =
    incoming.get("col","") if not bins or not col: raise Exception, "You
    need to specify a number for bins and columns" try: bins = int(bins)
    col = int(col) except: raise Exception, "Parameters are not integers,
    columns:%s, bins:%s" % (col, bins) if not 1&lt;bins&lt;100: raise
    Exception, "The number of bins %s must be a number between 1 and 100" %
    bins ``` This code will intercept a number of parameter errors and
    return corresponding error messages. The parameter ``incoming``
    ...
    **New name**. The difference between these two functions is that the
    ...
    :ivar file: This value is the name of the executable code file, and
        is called in the exec_before_process(), exec_before_job(),
        exec_after_process() and exec_after_job()( methods.
    """
...
class Column:
    ...
    &lt;/options&gt; ``` The
    [interval2maf](https://github.com/galaxyproject/galaxy/blob/dev/tools/maf/interval2maf.xml)
...
class Command:
    ...
    ``` A few things to note about even this simple example: * Input and
    output variables (boringly named ``input`` and ``output``) are expanded
    into paths using the ``$`` Cheetah directive. * Paths should be quoted
    so that the Galaxy database files may contain spaces. * We are building
    up a shell script - so special characters like ``&gt;`` can be used (in
    this case the standard output of the bedtools call is written to the
    path specified by ``'$output'``). The bed12ToBed6 tool can be found
    [here](https://github.com/galaxyproject/tools-iuc/blob/master/tools/bedtools/bed12ToBed6.xml).
    ...
    '${bed.inputName}' #end for #else: #set files = '" "'.join( [ str(
    $file ) for $file in $names.beds ] ) -files '${files}' #set names = '"
    "'.join( [ str( $name.display_name ) for $name in $names.beds ] )
    -names '${names}' #end if $strand $counts $both &gt; "${output}"
    ]]&gt;&lt;/command&gt; ``` The following example (taken from
    [xpath](https://github.com/galaxyproject/tools-iuc/blob/master/tools/xpath/xpath.xml)
    ...
    referenced with ``$__tool_directory__``. ```xml &lt;command&gt; perl
    $__tool_directory__/xpath -q -e '$expression' '$input' &gt; '$output'
    &lt;/command&gt; ``` The following example demonstrates accessing
    metadata from datasets. Metadata values (e.g.,
    ``${input.metadata.chromCol}``) are acquired from the ``Metadata``
    model associated with the objects selected as the values of each of the
    relative form field parameters in the tool form. Accessing this
    information is generally enabled using the following feature
    components. A set of "metadata information" is defined for each
    ...
    ``$__user_email__`` | User's email address ``$__app__`` | The
    ...
    when appearing in ``command`` or ``configfile`` elements. Name |
    Description ---- | ----------- ``\\${GALAXY_SLOTS:-4}`` | Number of
    cores/threads allocated by the job runner or resource manager to the
    ...
    an older Galaxy runtime). See the [Planemo
    ...
    Attributes #### ``detect_errors`` If present on the ``command`` tag,
    this attribute can be one of: * ``default`` no-op fallback to ``stdio``
    tags and erroring on standard error output (for legacy tools). *
    ``exit_code`` error if tool exit code is not 0. (The @jmchilton
    recommendation). * ``aggressive`` error if tool exit code is not 0 or
    either ``Exception:`` or ``Error:`` appears in standard error/output.
    (The @bgruening recommendation). For newer tools with
    ``profile&gt;=16.04``, the default behavior is ``exit_code``. Legacy
    tools default to ``default`` behavior described above (erroring if the
    tool produces any standard error output). See [PR
    117](https://github.com/galaxyproject/galaxy/pull/117) for more
    implementation information and discussion on the ``detect_errors``
    attribute. #### ``strict`` This boolean forces the ``#set -e``
    ...
    default for tools with ``profile&gt;=16.04`` and disabled on legacy
    tools. #### ``interpreter`` Older tools may define an ``intepreter``
    attribute on the command, but this is deprecated and using the
    ``$__tool_directory__`` variable is superior.

    :ivar value:
    :ivar detect_errors:
    :ivar interpreter: This attribute defines the programming language
        in which the tool's executable file is written. Any language can
        be used (tools can be written in Python, C, Perl, Java, etc.).
        The executable file must be in the same directory of the XML
        file. If instead this attribute is not specified, the tag
        content should be a Bash command calling executable(s) available
        in the $PATH.
    :ivar strict:
    """

    value: str = field(default="")
    detect_errors: None | str = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
...
class ConfigFile:
    ...
    options( show.error.messages=F, error = function () { cat(
    geterrmessage(), file=stderr() ); q( "no", 1, F ) } ) ## Determine
    range of all series in the plot xrange = c( NULL, NULL ) yrange = c(
    NULL, NULL ) #for $i, $s in enumerate( $series ) s${i} = read.table(
    "${s.input.file_name}" ) x${i} = s${i}[,${s.xcol}] y${i} =
    s${i}[,${s.ycol}] xrange = range( x${i}, xrange ) yrange = range(
    y${i}, yrange ) #end for ## Open output PDF file pdf( "${out_file1}" )
    ## Dummy plot for axis / labels plot( NULL, type="n", xlim=xrange,
    ylim=yrange, main="${main}", xlab="${xlab}", ylab="${ylab}" ) ## Plot
    each series #for $i, $s in enumerate( $series ) #if
    $s.series_type['type'] == "line" lines( x${i}, y${i},
...
class ConfigInputs:
    ...
    parameters . *Example* The following will create a cheetah variable
    that can be evaluated as ``$inputs`` that will contain the tool
    parameter inputs. ```xml &lt;configfiles&gt; &lt;inputs name="inputs"
    /&gt; &lt;configfiles&gt; ``` The following will instead write the
    inputs to the tool's working directory with the specified name (i.e.
...
class OutputFilter:
    """
    The ``&lt;data&gt;`` tag can contain a ``&lt;filter&gt;`` tag which
    includes a Python code block to be executed to test whether to include
    this output in the outputs the tool ultimately creates.

    If the code, when executed, returns ``True``, the output dataset is
    retained. In these code blocks the tool parameters appear as Python
    variables and are thus referred to without the $ used for the Cheetah
    template (used in the ``&lt;command&gt;`` tag). Variables that are part
    of conditionals are accessed using a hash named after the conditional.
    ### Example ```xml &lt;inputs&gt; &lt;param type="data" format="fasta"
    name="reference_genome" label="Reference genome" /&gt; &lt;param
    type="data" format="bam" name="input_bam" label="Aligned reads" /&gt;
    &lt;conditional name="options"&gt; &lt;param label="Use advanced
    options" name="selection_mode" type="select"&gt; &lt;option
    selected="True" value="defaults"&gt;Use default options&lt;/option&gt;
    &lt;option value="advanced"&gt;Use advanced options&lt;/option&gt;
    &lt;/param&gt; &lt;when value="defaults"&gt; &lt;/when&gt; &lt;when
    value="advanced"&gt; &lt;param name="vcf_output" type="boolean"
    checked="false" label="VCF output" truevalue="--vcf" falsevalue=""
    /&gt; &lt;/when&gt; &lt;conditional&gt; &lt;/inputs&gt; &lt;outputs&gt;
    &lt;data format="txt" label="Alignment report on ${on_string}"
    name="output_txt" /&gt; &lt;data format="vcf" label="Variant summary on
    ${on_string}" name="output_vcf"&gt;
...
class RequestParameterAppendValue:
    """
    Contained within the &lt;append_param&gt; tag set - allows for
    appending a param name / value pair to the value of URL.
    ...
    :ivar name: Any valid HTTP request parameter name. The name / value
        ...
        appended to the value of URL as something like "&amp;_export=1"
        (e.g. ``name="_export"``).
...
class RequestParameterValueTranslationValue:
    """
    Contained within the &lt;value_translation&gt; tag set - allows for
    changing the data type value to something supported by Galaxy.
...
class SanitizerValidAdd:
    ...
    Character must not be allowed as a valid input for the mapping to
    occur. Preset lists include default and none as well as those available
    from string.* (e.g. ``string.printable``).

    :ivar preset: Add target characters from the list of valid
        characters (e.g. ``string.printable``).
...
class SanitizerValidRemove:
    ...
    :ivar preset: Remove characters from the list of valid characters
        (e.g. ``string.printable``).
...
class TestAssertions:
    ...
    :ivar has_text: Asserts the specified ``text`` appears in the output
        (e.g. ``&lt;has_text text="chr7"&gt;``).
    :ivar not_has_text: Asserts the specified ``text`` does not appear
        in the output (e.g. ``&lt;not_has_text text="chr8" /&gt;``).
    :ivar has_text_matching: Asserts text matching the specified regular
        expression (``expression``) appears in the output (e.g.
        ``&lt;has_text_matching expression="1274\\d+53" /&gt;`` ).
    :ivar has_line: Asserts a line matching the specified string
        (``line``) appears in the output (e.g. ``&lt;has_line line="A
        full example line." /&gt;``).
    :ivar has_line_matching: Asserts a line matching the specified
        regular expression (``expression``) appears in the output (e.g.
        ``&lt;has_line_matching
        expression=".*\\s+127489808\\s+127494553" /&gt;``).
    :ivar has_n_columns: Asserts tabular output contains the specified
        number (``n``) of columns (e.g. ``&lt;has_n_columns n="3"
        /&gt;``).
    ...
    :ivar element_text_is: Asserts the text of the XML element with the
        specified XPath-like ``path`` is the specified ``text`` (e.g.
    ...
    :ivar attribute_matches: Asserts the XML ``attribute`` for the
        ...
        the regular expression specified by ``expression`` (e.g.
        ``&lt;attribute_matches path="outerElement/innerElement2"
        attribute="foo2" expression="bar\\d+" /&gt;``).
    :ivar element_text: This tag allows the developer to recurisively
        ...
        ``path`` (e.g. ``&lt;element_text
        path="BlastOutput_iterations/Iteration/Iteration_hits/Hit/Hit_def"&gt;&lt;not_has_text
        text="EDK72998.1" /&gt;&lt;/element_text&gt;``).
    """

    has_text: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    not_has_text: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_text_matching: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_line: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_line_matching: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_n_columns: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    is_valid_xml: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_element_with_path: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_n_elements_with_path: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    element_text_is: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    element_text_matches: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    attribute_is: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    attribute_matches: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    element_text: list[object] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
...
class TestCompositeData:
    ...
    :ivar ftype: Optional datatype of composite file for test input.
    """
...
class TestOutputCompareType(Enum):
    ...
    ``sim_size``, ``re_match_multiline``, and ``contains``.
    """
...
class TestOutputMetadata:
    ...
    :ivar name: Name of the metdata element to check.
...
class TestParamMetadata:
    """
    :ivar name: Documentation for name
    :ivar value: Documentation for value
    """
...
class VersionCommand:
    ...
    $__tool_directory__/count_reads.py&lt;/version_command&gt; ``` Examples
    are included in the test tools directory including: -
    ...
    :ivar value:
    :ivar interpreter: *Deprecated*: Prefix the version command with an
        interpreter and the tool directory in order to execute a script
        shipped with the tool. It is better to just use
        ``&lt;interpreter&gt; $__tool_directory__/&lt;script_name&gt;``.
    """
...
class ActionsConditionalFilter:
    ...
    keep: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    ...
    strip: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
...
class ChangeFormat:
    ...
    or the test tool
    [output_format.xml](https://github.com/galaxyproject/galaxy/blob/dev/test/functional/tools/output_format.xml)
    for simple examples of how this tag set is used in a tool.

    This tag set is optionally contained within the ``&lt;data&gt;`` tag
    set and is the container tag set for the following ``&lt;when&gt;`` tag
    set.
    """
...
class Citation:
    ...
    elements - each of which specifies tool citation information using
    either a DOI or a BibTeX entry.

    :ivar value:
    :ivar type_value: Type of citation - currently ``doi`` and
...
class Container:
    ...
    Galaxy can be configured to run tools within Docker
    (https://www.docker.com/) containers - this tag allows the tool to
    suggest possible valid Docker containers for this tool. Read more about
    configuring Galaxy to run Docker jobs
    [here](https://wiki.galaxyproject.org/Admin/Tools/Docker).

    :ivar value:
    :ivar type_value: This value describes the type of container that
        the tool may be executed in and currently must be 'docker'.
    """
...
class ExitCode:
    ...
    error level is either a warning or a fatal error. A warning means that
    stderr will be updated with the error's description. A fatal error
    means that the tool's execution will be marked as having an error and
    the workflow will stop. Note that, if the error level is not supplied,
    then a fatal error is assumed to have occurred. The exit code's range
    can be any consecutive group of integers. More advanced ranges, such as
    noncontiguous ranges, are currently not supported. Ranges can be
    specified in the form "m:n", where m is the start integer and n is the
    end integer. If ":n" is specified, then the exit code will be compared
    against all integers less than or equal to n. If "m:" is used, then the
    exit code will be compared against all integers greater than or equal
    to m. If the exit code matches, then the error level is applied and the
    error's description is added to stderr. If a tool's exit code does not
    match any of the supplied &lt;exit_code&gt; tags' ranges, then no
    errors are applied to the tool's execution. Note that most Unix and
    Linux variants only support positive integers 0 to 255 for exit codes.
    If an exit code falls out of the range 0 to 255, the usual convention
    is to only use the lower 8 bits for the exit code. The only known
    exception is if a job is broken into subtasks using the tasks runner
    and one of those tasks is stopped with a POSIX signal. (Note that
    signals should be used as a last resort for terminating processes.) In
    those cases, the task will receive -1 times the signal number. For
    example, suppose that a job uses the tasks runner and 8 tasks are
    created for the job. If one of the tasks hangs, then a sysadmin may
    choose to send the "kill" signal, SIGKILL, to the process. In that
    case, the task (and its job) will exit with an exit code of -9. More on
    POSIX signals can be found at https://en.wikipedia.org/wiki/Unix_signal
    as well as man pages on "signal". The &lt;exit_code&gt; tag's supported
    attributes are as follows: * ``range``: This indicates the range of
    exit codes to check. The range can be one of the following: * ``n``:
    the exit code will only be compared to n; * ``[m:n]``: the exit code
    must be greater than or equal to m and less than or equal to n; *
    ``[m:]``: the exit code must be greater than or equal to m; * ``[:n]``:
    the exit code must be less than or equal to n. * ``level``: This
    indicates the error level of the exit code. The level can have one of
    two values: * ``warning``: If an exit code falls in the given range,
    then a description of the error will be added to the beginning of
    stderr. A warning-level error will not cause the tool to fail. *
    ``fatal``: If an exit code falls in the given range, then a description
    of the error will be added to the beginning of stderr. A fatal-level
    error will cause the tool to fail. If no level is specified, then the
    fatal error level will be assumed to have occurred. * ``description``:
    This is an optional description of the error that corresponds to the
    exit code. The following is an example of the &lt;exit_code&gt; tag:
    ```xml &lt;stdio&gt; &lt;exit_code range="2" level="fatal"
    description="Out of Memory" /&gt; &lt;exit_code range="3:5"
    level="warning" description="Low disk space" /&gt; &lt;exit_code
    range="6:" level="fatal" description="Bad input dataset" /&gt;
    &lt;/stdio&gt; ``` If the tool returns 0 or 1, then the tool will not
    be marked as having an error. If the exit code is 2, then the tool will
    fail with the description ``Out of Memory`` added to stderr. If the
    tool returns 3, 4, or 5, then the tool will not be marked as having
...
class Filter:
    """
    Optionally contained within an ``&lt;options&gt;`` tag set - filter out
    values obtained from a locally stored file (e.g. a tool data table) or
    a dataset in the current history. ### Examples The following example
    from Mothur's
    ...
    name="taxons" type="select" size="120" optional="true" multiple="true"
    ...
    &lt;filter column="2" type="sort_by" /&gt; &lt;validator message="No
    genomes are available for the selected input dataset" type="no_options"
    /&gt; &lt;/options&gt; &lt;/param&gt; ``` The
    ...
    :ivar type_value: These values are defined in the module
        ...
        in the ``filter_types`` dictionary. Currently defined values
        are: ``data_meta``, ``param_value``, ``static_value``,
        ``unique_value``, ``multiple_splitter``,
        ``attribute_value_splitter``, ``add_value``, ``remove_value``,
        and ``sort_by``
    :ivar column: Column targeted by this filter - this attribute is
        unused and invalid if ``type`` is ``add_value`` or
        ``remove_value``. This can be a column index or a column name.
    ...
    :ivar keep: If ``true``, keep columns matching the value, if
        ...
        is either ``static_value`` or ``param_value``.
    :ivar value: Target value of the operations - has slightly different
        ...
        ``type`` is ``static_value`` it is the value compared against.
    ...
    multiple: PermissiveBoolean = field(
        default=PermissiveBoolean.FALSE,
        metadata={
            "type": "Attribute",
        },
    )
    ...
    keep: PermissiveBoolean = field(
        default=PermissiveBoolean.TRUE,
        metadata={
            "type": "Attribute",
        },
    )
...
class Options:
    ...
    :ivar refresh: Deprecated, likely unused attribute.
    :ivar sanitize: This attribute can be used to turn off all input
        sanitization for a tool.
    """

    refresh: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    )
    sanitize: PermissiveBoolean = field(
        default=PermissiveBoolean.TRUE,
        metadata={
            "type": "Attribute",
        },
...
class ParamOption:
    """
    See
    [/tools/filters/sorter.xml](https://github.com/galaxyproject/galaxy/blob/master/tools/filters/sorter.xml)
    ...
    with ``selected="true"`` to specify a default option. ```xml &lt;param
    name="col" type="select" label="From"&gt; &lt;option value="0"
    selected="true"&gt;Column 1 / Sequence name&lt;/option&gt; &lt;option
    value="1"&gt;Column 2 / Source&lt;/option&gt; &lt;option
    value="2"&gt;Column 3 / Feature&lt;/option&gt; &lt;option
    value="6"&gt;Column 7 / Strand&lt;/option&gt; &lt;option
    value="7"&gt;Column 8 / Frame&lt;/option&gt; &lt;/param&gt; ```.
    ...
    selected: PermissiveBoolean = field(
        default=PermissiveBoolean.FALSE,
        metadata={
            "type": "Attribute",
        },
...
class Regex:
    ...
    the follwing values: * ``stdout``: the regular expression will be
    ...
    stderr. If the &lt;regex&gt; tag does not contain the match attribute,
    then the &lt;regex&gt; tag will be ignored. The regular expression can
    be any valid Python regular expression. All regular expressions are
    performed case insensitively. For example, if match contains the
    regular expression "actg", then the regular expression will match
    against "actg", "ACTG", "AcTg", and so on. Also note that, if double
    quotes (") are to be used in the match attribute, then the value " can
    be used in place of double quotes. Likewise, if single quotes (') are
    to be used in the match attribute, then the value ' can be used if
    necessary. * ``level``: This works very similarly to the
    &lt;exit_code&gt; tag, except that, when a regular expression matches
    against its source, the description is added to the beginning of the
    source. For example, if stdout matches on a regular expression, then
    the regular expression's description is added to the beginning of
    stdout (instead of stderr). The level can be log, warning or fatal as
    described below. * ``log`` and ``warning``: If the regular expression
    matches against its source input (i.e., stdout and/or stderr), then a
    description of the error will be added to the beginning of the source,
    prepended with either 'Log:' or 'Warning:'. A log-level/warning-level
    error will not cause the tool to fail. * ``fatal``: If the regular
    expression matches against its source input, then a description of the
    error will be added to the beginning of the source. A fatal-level error
    will cause the tool to fail. If no level is specified, then the fatal
    error level will be assumed to have occurred. * ``description``: Just
    like its ``exit_code`` counterpart, this is an optional description of
    the regular expression that has matched. The following is an example of
    regular expressions that may be used: ```xml &lt;stdio&gt; &lt;regex
    match="low space" source="both" level="warning" description="Low space
    on device" /&gt; &lt;regex match="error" source="stdout" level="fatal"
    description="Unknown error encountered" /&gt; &lt;regex
    match="[CG]{12}" description="Fatal error - CG island 12 nts long
    found" /&gt; &lt;regex match="^Branch A" level="warning"
    ...
    the second regular expression did not match, then the third regular
    expression is checked. The third regular expression does not contain an
    error level, so an error level of ``fatal`` is assumed. The third
    regular expression also does not contain a source, so both stdout and
    stderr are checked. The third regular expression looks for 12
    consecutive "C"s or "G"s in any order and in uppercase or lowercase. If
    stdout contained ``cgccGGCCcGGcG`` or stderr contained
    ...
    of the fatal errors, then the fourth regular expression is checked.
    ...
    :ivar source: This tells whether the regular expression should be
        ...
        checked. The source can be one of the follwing values:
...
class RequestParameterAppend:
    """
    Optionally contained within the &lt;request_param&gt; tag set if
    galaxy_name="URL" - some remote data sources ( e.g., Gbrowse, Biomart )
    send parameters back to Galaxy in the initial response that must be
    added to the value of "URL" prior to Galaxy sending the secondary
    request to the remote data source via URL.
...
class RequestParameterValueTranslation:
    """
    Optionally contained within the &lt;request_param&gt; tag set the
    parameter value received from a remote data source may be named
...
class Requirement:
    ...
    be abstract and resolvable by multiple different systems (e.g.
    [conda](http://conda.pydata.org/docs/), the [Galaxy Tool Shed
    dependency management
    system](https://wiki.galaxyproject.org/ToolShedToolFeatures#Automatic_third-party_tool_dependency_installation_and_compilation_with_installed_repositories),
    or [environment modules](http://modules.sourceforge.net/)). Read more
    about dependency resolvers in Galaxy on
    ...
    ### Examples This example shows a tool that requires the samtools
    0.0.18 package. This package is available via the Tool Shed (see [Tool
    Shed dependency
    management](https://wiki.galaxyproject.org/ToolShedToolFeatures#Automatic_third-party_tool_dependency_installation_and_compilation_with_installed_repositories)
    ) as well as
    [Conda](https://docs.galaxyproject.org/en/master/admin/conda_faq.html)
    and can be configured locally to adapt to any other package management
    system. ```xml &lt;requirements&gt; &lt;requirement type="package"
    version="0.1.18"&gt;samtools&lt;/requirement&gt; &lt;/requirements&gt;
    ``` This older example shows a tool that requires R version 2.15.1. The
    ``tool_depensencies.xml`` should contain matching declarations for
    ...
    :ivar value:
    :ivar type_value: This value defines the which type of the 3rd party
        module required by this tool.
    :ivar version: For package type requirements this value defines a
...
class SanitizerValid:
    ...
    :ivar initial: This describes the initial characters to allow as
        valid, the default is ``string.letters + string.digits + "
        -=_.()/+*^,:?!"``
    """
...
class TestOutput:
    ...
    :ivar assert_contents: $assertions ### Examples The following
        demonstrtes a wide variety of text-based and tabular assertion
        ...
        &lt;/output&gt; ``` The following demonstrtes a wide variety of
        ...
        demonstrtes verifying XML content with XPath-like expressions.
    ...
    :ivar ftype: If specified, this value will be checked against the
        ...
        test will fail.
    :ivar sort: This flag causes the lines of the output to be sorted
        before they are compared to the expected output. This could be
    ...
    :ivar md5: If specified, the target output's MD5 hash should match
        ...
        inconvenient to upload the entiry file and this can be used
        instead.
    :ivar checksum: If specified, the target output's checksum should
        ...
        static files it may be inconvenient to upload the entiry file
        and this can be used instead.
    :ivar compare:
    :ivar lines_diff: If ``compare`` is set to ``diff``, the number of
        lines of difference to allow (each line with a modification is a
        line added and a line removed so this counts as two lines).
    :ivar decompress: If ``compare`` is set to ``diff``, attempt to
        decompress both produced output and expected output files if
        needed before performing the diff. This is useful for testing
        gzipped outputs that are non-deterministic despite having
        deterministic decompressed contents. This is available in Galaxy
        17.05+ and was introduced in ``Pull Request #3550
        &lt;https://github.com/galaxyproject/galaxy/pull/3550&gt;`__.
    :ivar delta: If ``compare`` is set to ``sim_size``, this is the
        number of bytes different allowed.
    """
    ...
    sort: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    ...
    decompress: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
...
class TracksterConf:
    """
    This directive is used to specify some rarely modified trackster
    options.
    """

    action: list[TracksterAction] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
...
class Validator:
    ...
    This tag set is contained within the &lt;param&gt; tag set - it applies
    a validator to the containing parameter. ### Examples The following
    ...
    type="dataset_metadata_in_file" filename="alignseq.loc"
    metadata_name="dbkey" metadata_column="1" message="Sequences are not
    currently available for the specified build." split=" "
    line_startswith="seq" /&gt; ``` A very common validator is simply
    ...
    :ivar value:
    :ivar type_value: The list of supported validators is in the
        ``validator_types`` dictionary in
        ...
        Valid values include: ``expression``, ``regex``, ``in_range``,
        ``length``, ``metadata``, ``unspecified_build``, ``no_options``,
        ``empty_field``, ``dataset_metadata_in_file``,
        ``dataset_metadata_in_data_table``, ``dataset_ok_validator``
    ...
    :ivar table_name: Tool data table name to check against if ``type``
        is ``dataset_metadata_in_tool_data``. See the documentation for
        [tool data
        tables](https://wiki.galaxyproject.org/Admin/Tools/Data%20Tables)
        and [data
        managers](https://wiki.galaxyproject.org/Admin/Tools/DataManagers)
        for more information.
    :ivar filename: Tool data filename to check against if ``type`` is
        ``dataset_metadata_in_file``. File should be present Galaxy's
        ``tool-data`` directory.
    :ivar metadata_name: Target metadata attribute name for
        ``dataset_metadata_in_data_table`` and
        ``dataset_metadata_in_file`` options.
    :ivar metadata_column: Target column for metadata attribute in
        ``dataset_metadata_in_data_table`` and
        ``dataset_metadata_in_file`` options. This can be an integer
        index to the column or a column name.
    :ivar line_startswith: Used to indicate lines in the file being used
        for validation start with a this attribute value.
    :ivar min: When the ``type`` attribute value is ``in_range`` - this
        is the minimum number allowed.
    :ivar max: When the ``type`` attribute value is ``in_range`` - this
        is the maximum number allowed.
    :ivar exclude_min: When the ``type`` attribute value is ``in_range``
        - this boolean indicates if the ``min`` value is allowed.
    :ivar exclude_max: When the ``type`` attribute value is ``in_range``
...
class ActionsOption:
    ...
    type_value: None | ActionsOptionType = field(
        default=None,
        metadata={
            "name": "type",
            "type": "Attribute",
...
class Citations:
    ...
    each of which specifies tool citation information using either a DOI or
    a BibTeX entry. These citations will appear at the bottom of the tool
    form in a formatted way but the user will have to option to select RAW
    BibTeX for copying and pasting as well. Likewise, the history menu
    includes an option allowing users to aggregate all such citations
    across an analysis in a list of citations. BibTeX entries for citations
    annotated with DOIs will be fetched by Galaxy from https://dx.doi.org/
    and cached. ```xml &lt;citations&gt; &lt;!-- Example of annotating a
    citation using a DOI. --&gt; &lt;citation
    ...
    This feature was added to the August 2014 release of Galaxy, tools
    annotated with citations will work in older releases of Galaxy but no
    citation information will be available to the end user.
    """
...
class ParamOptions:
    """
    See
    [/tools/extract/liftOver_wrapper.xml](https://github.com/galaxyproject/galaxy/blob/master/tools/extract/liftOver_wrapper.xml)
    ...
    dynamically generated lists of options. This tag set dynamically
    creates a list of options whose values can be obtained from a
    predefined file stored locally or a dataset selected from the current
    history. There are at least five basic ways to use this tag - four of
    these correspond to a ``from_XXX`` attribute on the ``options``
    ...
    manager. * ``from_dataset`` - The options for the select list are
    ...
    dynamically obtained from a file. This mechanis is discourage in favor
    of the more generic ``from_data_table``. * ``from_parameter`` - The
    options for the select list are dynamically obtained from a parameter.
    * Using ``filter``s - various filters can be used to populate options,
    see examples in the [filter](#tool-inputs-param-options-filter)
    documentation. ### ``from_data_table`` See Galaxy's [data tables
    documentation](https://wiki.galaxyproject.org/Admin/Tools/Data%20Tables)
    for information on setting up data tables. Once a data table has been
    ...
    type="sort_by" column="2"/&gt; &lt;validator type="no_options"
    message="No indexes are available for the selected input dataset"/&gt;
    &lt;/options&gt; &lt;/param&gt; &lt;/when&gt; &lt;when
    ...
    size="120" optional="true" multiple="true" label="Browse Taxons from
    Taxonomy"&gt; &lt;options from_dataset="taxonomy"&gt; &lt;column
    name="name" index="1"/&gt; &lt;column name="value" index="1"/&gt;
    &lt;filter type="unique_value" name="unique_taxon" column="1"/&gt;
    &lt;filter type="sort_by" name="sorted_taxon" column="1"/&gt;
    &lt;/options&gt; &lt;sanitizer&gt; &lt;valid initial="default"&gt;
    &lt;add preset="string.printable"/&gt; &lt;add value=";"/&gt;
    &lt;remove value="&amp;quot;"/&gt; &lt;remove value="&amp;apos;"/&gt;
    &lt;/valid&gt; &lt;/sanitizer&gt; &lt;/param&gt; ``` Filters can be
    used to generate options from dataset directly also as the example
    below demonstrates (many more examples are present in the
    ...
    :ivar from_dataset: Documentation for from_dataset
    :ivar from_file: Documentation for from_file
    :ivar from_data_table: Documentation for from_data_table
    :ivar from_parameter: Documentation for from_parameter
    :ivar options_filter_attribute: Documentation for
        options_filter_attribute
    :ivar transform_lines: Documentation for transform_lines
    :ivar startswith: Documentation for startswith
    """
...
class RequestParameter:
    """
    Contained within the &lt;request_param_translation&gt; tag set ( used
    only in "data_source" tools ) - the external data source application
    may send back parameter names like "GENOME" which must be translated to
    "dbkey" in Galaxy.
    ...
    galaxy_name: None | RequestParameterGalaxyNameType = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    )
    remote_name: None | str = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
...
class Requirements:
    """
    This is a container tag set for the ``requirement`` and ``container``
    tags described in greater detail below. ``requirement``s describe
    software packages and other individual computing requirements required
    to execute a tool, while ``container``s describe Docker containers that
    should be able to serve as complete descriptions of the runtime of a
    tool.
    """
...
class Sanitizer:
    ...
    ``&lt;mapping&gt;`` tags. ### Examples This example replaces the
    invalid character default of ``X`` with the empty string (so invalid
    characters are effectively dropped instead of replaced with ``X``) and
    indicates the only valid characters for this input are ASCII letters,
    ASCII digits, and ``_``. ``` &lt;param name="mystring" type="text"
    label="Say something interesting"&gt; &lt;sanitizer invalid_char=""&gt;
    &lt;valid initial="string.letters,string.digits"&gt;&lt;add value="_"
    /&gt; &lt;/valid&gt; &lt;/sanitizer&gt; &lt;/param&gt; ``` This example
    allows many more valid characters and specifies that ``&amp;`` will
    just be dropped from the input. ``` &lt;sanitizer&gt; &lt;valid
...
class Stdio:
    ...
    other than 0 as an error. The ``detect_error`` on ``command`` can swap
...
class TestDiscoveredDataset(TestOutput):
    ...
    provides a demonstration of using this tag. ```xml &lt;test&gt;
    &lt;param name="num_param" value="7" /&gt; &lt;param name="input"
    ftype="txt" value="simple_line.txt"/&gt; &lt;output name="sample"&gt;
...
class TestOutputCollection:
    ...
    Expectations about collecton contents are described using child
    ``element`` directives. For nested collections, these child ``element``
    directives may themselves contain children. ### Examples The
    [genetrack](https://github.com/galaxyproject/tools-iuc/blob/master/tools/genetrack/genetrack.xml)
    ...
    [CWPair2](https://github.com/galaxyproject/tools-iuc/blob/master/tools/cwpair2/cwpair2.xml)
    ...
    :ivar element:
    :ivar name: This value is the same as the value of the ``name``
        attribute of the ``&lt;collecton&gt;`` tag set contained within
        the tool's ``&lt;outputs&gt;`` tag set.
    :ivar type_value: Expected collection type (e.g. ``list``,
        ``paired``, or ``list:paired``).
...
class Param(InputType):
    ...
    create a text box 30 characters wide with the default value of "V1".
    ```xml &lt;param name="xlab" size="30" type="text" value="V1"
    label="Label for x axis"/&gt; ``` The ``size`` parameter can be two
    dimensional, if it is the textbox will be rendered on the tool form as
    a text area instead of a single line text box. ```xml &lt;param
    name="foo" type="text" area="True" size="5x25" /&gt; ``` As of 17.01,
    ``text`` parameters can also supply a static list of preset defaults
    options. The user **may** be presented with the option to select one of
    these but will be allowed to supply an arbitrary text value. ```xml
    &lt;param name="foo" type="text" value="foo 1"&gt; &lt;option
    value="foo 1"&gt;Foo 1 Display&lt;/option&gt; &lt;option value="foo
    2"&gt;Foo 2 Display&lt;/option&gt; &lt;/param&gt; ``` See
    ...
    real numbers, respectively. ##### Example ``` &lt;param
    name="region_size" size="4" type="integer" value="1" label="flanking
    regions of size" /&gt; ``` $attribute_list:value,min,max:5 ####
    ``boolean`` This represents a binary true or false value.
    ...
    on this topic. $attribute_list:format,multiple:5 #### ``select`` The
    ...
    parameter. $attribute_list:force_select,numerical,use_header_name:5
    #### ``drill_down`` $attribute_list:hierarchy:5 ####
    ``data_collection`` The following will create a parameter that only
    accepts paired FASTQ files grouped into a collection. ##### Examples
    ```xml &lt;param name="inputs" type="data_collection"
    collection_type="paired" label="Input FASTQs" format="fastq"&gt;
    &lt;/param&gt; ``` More detailed information on writing tools that
    consume collections can be found in the [planemo
    ...
    initial="string.letters,string.digits"&gt; &lt;add value="#" /&gt;
    &lt;/valid&gt; &lt;/sanitizer&gt; &lt;/param&gt; ```
    $attribute_list:value,rgb:5 This covers examples of the most common
    parameter types, the remaining parameter types are more obsecure and
    less likely to be useful for most tool authors.
    ...
    :ivar type_value: Describes the parameter type - each different type
        ...
        ``data_collection``,  ``library_data``, ``drill_down``. The
        definition of supported parameter types as defined in the
    ...
    :ivar area: Boolean indicating if this should be rendered as a one
        line text box (if ``false``) or a multi-line text area (if
        ``true``).
    :ivar argument: If the parameter reflects just one command line
        ...
        help section, and it will create the name attribute from the
        argument attribute by stripping the dashes (e.g. if ``argument="
        --sensitive"`` then ``name="sensitive"`` is implicit).
    ...
    :ivar default_value: Deprecated way to specify default value for
        column parameters (use ``value`` instead).
    :ivar optional: If ``false``, parameter must have a value. Defaults
        to "false".
    :ivar rgb: If ``false``, the returned value will be in Hex color
        code. If ``true`` it will be a RGB value e.g. 0,0,255. This
        attribute is only valid when ``type`` is ``color``.
    :ivar min: Minimum valid parameter value - only valid when ``type``
        is ``integer`` or ``float``.
    :ivar max: Maximum valid parameter value - only valid when ``type``
        is ``integer`` or ``float``.
    :ivar format: Only if ``type`` attribute value is ``data`` or
        ``data_collection`` - the list of supported data formats is
        contained in the
        [/config/datatypes_conf.xml.sample](https://github.com/galaxyproject/galaxy/blob/dev/config/datatypes_conf.xml.sample)
        file. Use the file extension.
    :ivar collection_type: This is only valid if ``type`` is
        ``data_collection``. Restrict the kind of collection that can be
        consumed by this parameter (e.g. ``paired``, ``list:paired``,
        ``list``). Multiple such collection types can be specified here
        as a comma separated list.
    :ivar data_ref: Only valid if ``type`` attribute value is ``select``
        or ``data_column``. Used with select lists whose options are
        dynamically generated based on certain metadata attributes of
        the dataset upon which this parameter depends (usually but not
        always the tool's input dataset).
    :ivar accept_default:
    ...
    :ivar force_select: Used only if the ``type`` attribute value is
        ``data_column``, this is deprecated and the inverse of
        ``optional``. Set to ``false`` to not force user to select an
        option in the list.
    :ivar use_header_names: Used only if the ``type`` attribute value is
        ``data_column``, if ``true`` Galaxy assumes first row of
        ``data_ref`` is a header and builds the select list with these
        values rather than the more generic ``c1`` ... ``cN``.
    :ivar display: This attribute is used only if ``type`` attribute
        value is ``select`` - render a select list as a set of check
        boxes or radio buttons. Defaults to a drop-down menu select
        list.
    :ivar multiple: Allow multiple valus to be selected. Valid with
        ``data`` and ``select`` parameters.
    :ivar numerical: Used only if the ``type`` attribute value is
        ``data_column``, if ``true`` the column will be treated as
        numerical when filtering columns based on metadata.
    :ivar hierarchy: Used only if the ``type`` attribute value is
        ``drill_down``, this attribute determines the drill down is
        ``recursive`` or ``exact``.
    :ivar checked: Set to ``true`` if the ``boolean`` parameter should
        be checked (or ``true``) by default.
    :ivar truevalue: The parameter value in the Cheetah template if the
        parameter is ``true`` or checked by the user. Only valid if
        ``type`` is ``boolean``.
    :ivar falsevalue: The parameter value in the Cheetah template if the
        parameter is ``false`` or not checked by the user. Only valid if
        ``type`` is ``boolean``.
    :ivar size: Used only if ``type`` attribute value is ``text``. To
        create a multi-line text box add an ``area="True"`` attribute to
        the param tag. This can be one dimensional (e.g. ``size="40"``)
    ...
    area: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    ...
    accept_default: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    )
    refresh_on_change: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    )
    force_select: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    )
    use_header_names: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    ...
    multiple: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    )
    numerical: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
    ...
    checked: PermissiveBoolean = field(
        default=PermissiveBoolean.FALSE,
        metadata={
            "type": "Attribute",
        },
...
class RequestParameterTranslation:
    """
    See ~/tools/data_source/ucsc_tablebrowser.xml for an example of how to
    use this tag set.

    This tag set is used only in "data_source" tools (the "tool_type"
    attribute value is "data_source"). This tag set is contained within the
    &lt;param&gt; tag set - it contains a set of &lt;request_param&gt;
    tags.
    """
...
class ActionsConditionalWhen:
    ...
    :ivar action:
    :ivar value: Value to match conditional input value against.
...
class Conditional(InputType):
    ...
    contain 'when' tag sets) - the command line (or portions thereof) are
    then wrapped in an if-else statement.

    A good example tool that demonstrates many conditional parameters is
    ...
    argument="--input-fp"/&gt; &lt;param name="process_obs_metadata"
    type="select" label="Process metadata associated with observations when
    converting" argument="--process-obs-metadata"&gt; &lt;option value=""
    selected="True"&gt;Do Not process metadata&lt;/option&gt; &lt;option
    ...
    instance via [data
    managers](https://wiki.galaxyproject.org/Admin/Tools/DataManagers) )
    and history files. A good example tool that demonstrates this is the
    [Bowtie
    2](https://github.com/galaxyproject/tools-devteam/blob/master/tools/bowtie2/bowtie2_wrapper.xml)
    ...
    column="2"/&gt; &lt;validator type="no_options" message="No indexes are
    available for the selected input dataset"/&gt; &lt;/options&gt;
    ...
    :ivar value_ref_in_group: Infrequently used option to dynamically
        ...
        parameter is the same group.
    :ivar label: Human readable description for the conditional, unused
        in the Galaxy UI currently.
    """

    param: None | Param = field(
        default=None,
        metadata={
            "type": "Element",
        },
    )
    repeat: None | Repeat = field(
        default=None,
        metadata={
            "type": "Element",
        },
    )
    conditional: None | Conditional = field(
        default=None,
        metadata={
            "type": "Element",
        },
    ...
    upload_dataset: None | object = field(
        default=None,
        metadata={
            "type": "Element",
        },
    )
    display: None | str = field(
        default=None,
        metadata={
            "type": "Element",
        },
    ...
    value_ref_in_group: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
...
class TestConditional:
    ...
    &lt;/output&gt; &lt;/test&gt; ``` The
    [tophat2](https://github.com/galaxyproject/tools-devteam/blob/master/tools/tophat2/tophat2_wrapper.xml)
...
class Repeat(InputType):
    ...
    ``&lt;configfile&gt;`` tag set. ```xml #for $i, $s in enumerate(
    $series ) rank_of_series=$i input_path='${s.input}' x_colom=${s.xcol}
...
class Actions:
    ...
    &lt;/actions&gt; &lt;/data&gt; ``` ### Unconditional Actions and Column
    Names For a static file that contains a fixed number of columns, it is
    straight forward: ```xml &lt;outputs&gt; &lt;data format="tabular"
    ...
    the ```&lt;filter type="metadata_value"... column="1" /&gt;`` tag
    does), then it should be changed to ``equCab2`` (which is the
    ``&lt;option type="from_param" ... column="0" ...&gt;`` tag does).
    ```xml &lt;actions&gt; &lt;conditional
    name="refGenomeSource.genomeSource"&gt; &lt;when value="indexed"&gt;
    &lt;action type="metadata" name="dbkey"&gt; &lt;option type="from_file"
    name="bowtie_indices.loc" column="0" offset="0"&gt; &lt;filter
    type="param_value" column="0" value="#" compare="startswith"
    keep="False"/&gt; &lt;filter type="param_value"
    ref="refGenomeSource.index" column="1"/&gt; &lt;/option&gt;
    &lt;/action&gt; &lt;/when&gt; &lt;/conditional&gt; &lt;!-- Special
    casing equCab2chrM to equCab2 --&gt; &lt;action type="metadata"
    name="dbkey"&gt; &lt;option type="from_param"
...
class Section:
    ...
    need to include the section name to access the variable: ``` --color
    $adv.plot_color ``` Further examples can be found in the [test
    case](https://github.com/galaxyproject/galaxy/blob/master/test/functional/tools/section.xml)
    from [PR #35](https://github.com/galaxyproject/galaxy/pull/35).
    ...
    expanded: PermissiveBoolean = field(
        default=PermissiveBoolean.FALSE,
        metadata={
            "type": "Attribute",
        },
...
class Test:
    ...
    :ivar expect_num_outputs: Assert the number of outputs this test
        should produce, this is useful to ensure ``filter`` directives
    ...
    expect_failure: PermissiveBoolean = field(
        default=PermissiveBoolean.FALSE,
        metadata={
            "type": "Attribute",
        },
...
class Data:
    ...
    :ivar format_source: This sets the data type of the output file to
        be the same format as that of a tool input dataset.
    :ivar metadata_source: This copies the metadata information from the
        tool's input dataset. This is particularly useful for interval
        data types where the order of the columns is not set.
    :ivar label: This will be the name of the history item for the
        ...
        ${on_string}``.
    :ivar from_work_dir: Relative path to a file produced by the tool in
        its working directory. Output's contents are set to this file's
        contents.
    ...
    auto_format: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
...
class Inputs:
    """
    Consists of all tag sets that define the tool's input parameters.

    Each ``&lt;param&gt;`` tag within the ``&lt;inputs&gt;`` tag set maps
    to a command line parameter within the [command](#tool-command) tag.
    Most tools will not need to specify any attributes on this tag itself.
    ...
    :ivar method: Data source HTTP action (e.g. ``get`` or ``put``) to
        use.
    ...
    check_values: PermissiveBoolean = field(
        default=PermissiveBoolean.TRUE,
        metadata={
            "type": "Attribute",
        },
    ...
    nginx_upload: PermissiveBoolean = field(
        default=PermissiveBoolean.FALSE,
        metadata={
            "type": "Attribute",
        },
...
class Collection:
    ...
    :ivar name: Name for this output. This ``name`` is used as the
        ...
        ``command`` and ``configfile`` elements. The name should not
        contain pipes or periods (e.g. ``.``).
    :ivar type_value: Collection type for output (e.g. ``paired``,
        ``list``, or ``list:list``).
    :ivar label: This will be the name of the history item for the
        output data set. The string can include structure like
        ``${&lt;some param name&gt;.&lt;some attribute&gt;}``, as
        ...
        tag set section above. The default label is ``${tool.name} on
        ${on_string}``.
    :ivar format_source: This is the name of input collection or dataset
        to derive output dataset collection's element's format/datatype
        from.
...
class Tool:
    ...
    Attributes on this tag apply to the tool as a whole. ### Examples A
    normal tool: ```xml &lt;tool id="seqtk_seq" name="Convert FASTQ to
    FASTA" version="1.0.0" profile="16.04" &gt; ``` A ``data_source`` tool
    contains a few more relevant attributes. ```xml &lt;tool
    id="ucsc_table_direct1" name="UCSC Main" version="1.0.0" hidden="false"
    profile="16.01" tool_type="data_source" URL_method="post"&gt; ```.
    ...
    :ivar command:
    :ivar request_parameter_translation:
    :ivar configfiles:
    :ivar outputs:
    :ivar inputs:
    :ivar tests:
    :ivar stdio:
    :ivar help: This tag set includes all of the necessary details of
        how to use the tool. This tag set should be included as the next
        to the last tag set, before citations, in the tool config. Tool
        help is written in reStructuredText. Included here is only an
        overview of a subset of features. For more information see
        http://docutils.sourceforge.net/docs/ref/rst/restructuredtext.html
        . tag | details --- | ------- ``.. class:: warningmark`` | a
        yellow warning symbol ``.. class:: infomark`` | a blue
        information symbol ``.. image:: path-of-the-file.png :height:
        500 :width: 600`` | insert a png file of height 500 and width
        600 at this position | ``**bold**`` | bold ``*italic*`` | italic
        ``*`` | list ``-`` | list ``::`` | paragraph ``-----`` | a
        horizontal line ### Examples Show a warning sign to remind users
        that this tool accept fasta format files only, followed by an
        example of the query sequence and a figure. ```xml &lt;help&gt;
        .. class:: warningmark '''TIP''' This tool requires *fasta*
        format. ---- '''Example''' Query sequence:: &gt;seq1 ATCG... ..
        image:: my_figure.png :height: 500 :width: 600 &lt;/help&gt; ```
    :ivar code:
    :ivar uihints:
    :ivar options:
    :ivar trackster_conf:
    ...
    :ivar name: This string is what is displayed as a hyperlink in the
        tool menu.
    :ivar version: This string defaults to ``1.0.0`` if it is not
        included in the tag. It allows for tool versioning and should be
        increased with each new version of the tool.
    ...
    :ivar tool_type: Allows for certain framework functionality to be
        ...
        Source](https://wiki.galaxyproject.org/Admin/Internals/DataSources)
        and [Data
        Manager](https://wiki.galaxyproject.org/Admin/Tools/DataManagers)
        tools should set this to have values such as ``data_source`` or
        ``manage_data``.
    :ivar profile: This string specified the minimum Galaxy version that
        ...
        version newer than ``16.01``, such as ``16.04``.
    ...
    :ivar url_method: Only used if ``tool_type`` attribute value is
        ``data_source`` - this attribute defines the HTTP request method
        to use when communicating with an external data source
        application (the default is ``get``).
    """

    class Meta:
        name = "tool"

    macros: None | object = field(
        default=None,
        metadata={
            "type": "Element",
        },
    ...
    request_parameter_translation: None | RequestParameterTranslation = field(
        default=None,
        metadata={
            "type": "Element",
        },
    ...
    trackster_conf: None | TracksterConf = field(
        default=None,
        metadata={
            "type": "Element",
        },
    ...
    hidden: PermissiveBoolean = field(
        default=PermissiveBoolean.FALSE,
        metadata={
            "type": "Attribute",
        },
    )
    display_interface: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
