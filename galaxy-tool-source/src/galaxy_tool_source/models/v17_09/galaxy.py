...
class Command:
    ...
    tool produces any standard error output). See [pull request
...
class Code:
    ...
    **Deprecated** Do not use this unless absolutely necessary.
    ...
    elements we're using look like this. ```python MetadataElement(
    name="field_names", default=[], desc="Field names", readonly=True,
    optional=True, visible=True, no_value=[] ) # The keys in the
    field_components map to the list of field_names in the above element #
    ...
    and components", readonly=True, optional=True, visible=True,
    no_value={} ) ``` Our tool config includes a code file tag like this.
    ```xml &lt;code file="tool_form_utils.py" /&gt; ``` Here are the
    relevant input parameters in our tool config. The first parameter is
    the input dataset that includes the above metadata elements. ```xml
...
class Container:
    ...
    [Docker](https://www.docker.com/) containers - this tag allows the tool
    to suggest possible valid Docker containers for this tool. Read more
    about configuring Galaxy to run Docker jobs
...
class Requirement:
    ...
    system](https://galaxyproject.org/toolshed/tool-features/#Automatic_third-party_tool_dependency_installation_and_compilation_with_installed_repositories),
    ...
    Shed dependency
    management](https://galaxyproject.org/toolshed/tool-features/#Automatic_third-party_tool_dependency_installation_and_compilation_with_installed_repositories)
    ) as well as
...
class TestOutput:
    ...
    :ivar decompress: If ``compare`` is set to ``diff``, attempt to
        ...
        needed before performing the diff. This flag is useful for
        testing gzipped outputs that are non-deterministic despite
        having deterministic decompressed contents. This is available in
        Galaxy 17.05+ and was introduced in [pull request
...
class Outputs:
    ...
    :ivar provided_metadata_style: Style used for tool provided metadata
        file (i.e.
        [galaxy.json](http://planemo.readthedocs.io/en/latest/writing_advanced.html#tool-
    ...
    :ivar provided_metadata_file: Path relative to tool's working
        ...
        [here](http://planemo.readthedocs.io/en/latest/writing_advanced.html#tool-
