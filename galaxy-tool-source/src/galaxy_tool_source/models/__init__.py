...
``v26_0``), produced at build time by ``galaxy_tool_source._codegen`` and not
committed. Resolve a version to its model package or ``Tool`` class through
``galaxy_tool_source.models.registry``; the ``AnyTool`` union over every version
lives in ``galaxy_tool_source.models.any_tool``.
