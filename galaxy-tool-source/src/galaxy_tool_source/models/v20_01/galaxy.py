...
class Command:
    ...
    * Paths should be quoted so that the Galaxy database files may contain
    ...
    :ivar value:
    :ivar detect_errors: One of ``default``, ``exit_code``,
        ``aggressive``. See "Error detection" above for details.
    ...
    :ivar strict: This boolean forces the ``#set -e`` directive on in
        ...
        by default for tools with ``profile&gt;=16.04`` and disabled on
        legacy tools.
    """
...
class OutputFilter:
    ...
    Cheetah. ### Example ```xml &lt;inputs&gt; &lt;param type="data"
...
class ExitCode:
    ...
    255 for exit codes. If an exit code falls out of the range 0 to 255,
    the usual convention is to only use the lower 8 bits for the exit code.
    The only known exception is if a job is broken into subtasks using the
    ...
    ``&lt;exit_code&gt;`` tag: ```xml &lt;stdio&gt; &lt;exit_code
    range="3:5" level="warning" description="Low disk space" /&gt;
    ...
    this. --&gt; &lt;exit_code range="2" level="fatal_oom" description="Out
    of Memory" /&gt; &lt;/stdio&gt; ``` If the tool returns 0 or 1, then
    the tool will not be marked as having an error. If the exit code is 2,
    then the tool will fail with the description ``Out of Memory`` added to
    stderr. If the tool returns 3, 4, or 5, then the tool will not be
    marked as having failed, but ``Low disk space`` will be added to
    stderr. Finally, if the tool returns any number greater than or equal
    to 6, then the description ``Bad input dataset`` will be added to
    stderr and the tool will be marked as having failed.
    """
...
class Filter:
    ...
    current history. ### Examples The following example from Mothur's
    ...
    :ivar type_value: Currently the following filters are defined: *
        ``static_value`` filter options for which the entry in a given
        ``column`` of the referenced file based on equality to the
        ``value`` attribute of the filter. * ``regexp`` similar to the
        ``static_value`` filter, but checks if the regular expression
        given by ``value`` matches the entry. * ``param_value`` filter
        options for which the entry in a given ``column`` of the
        referenced file based on properties of another input parameter
        specified by ``ref``. This property is by default the value of
        the parameter, but also the values of another attribute
        (``ref_attribute``) of the parameter can be used, e.g. the
        extension of a data input. * ``data_meta`` If a ``column`` is
        given options are filtered for which the entry in this column
        ``column`` is equal to metadata of another input parameter
        specified by ``ref``. If no ``column`` is given the metadata
        value of the referenced input is added to the options list (in
        this case the corresponding ``options`` tag must not have the
        ``from_data_table`` or ``from_dataset`` attributes). In both
        cases the desired metadata is selected by ``key``. The above
        filters can be inverted by setting ``keep`` to true. *
        ``add_value``: add an option with a given ``name`` and ``value``
        to the options. By default the new option is appended, with
        ``index`` the insertion position can be specified. *
        ``remove_value``: remove a value from the options. Either
        specified explicitly with ``value``, the value of another input
        specifified with ``ref``, or the metatdata ``key`` of another
        input ``meta_ref``. * ``unique_value``: remove options that have
        duplicate entries in the given ``column``. * ``sort_by``: sort
        options by the entries of a given ``column``. *
        ``multiple_splitter``: split the entries of the specified
        ``column``(s) in the referenced file using a ``separator``.
        Thereby the number of columns is increased. Note that filters
        that are applyed after one of the latter two filters must not
        refer to These values are defined in the module
        ...
        in the ``filter_types`` dictionary. Deprecated filter types: *
...
class Validator:
    ...
    applies a validator to the containing parameter. ### Examples The
    following demonstrates a simple validator ``unspecified_build``
    ensuring that a dbkey is present on the selected dataset. This example
    is taken from the
...
class Param(InputType):
    ...
    form. #### ``group_tag`` $attribute_list:multiple,date_ref:5 #####
    Examples The following will find all "coordinate interval files"
    contained within the current history and dynamically populate a select
    list with them. If they are selected, their destination and internal
    file name will be passed to the appropriate command line variable.
    ```xml &lt;param name="interval_file" type="data" format="interval"
    label="near intervals in"/&gt; ``` The following demonstrates a
    ``param`` which may accept multiple files and multiple formats. ```xml
    &lt;param format="sam,bam" multiple="true" name="bamOrSamFile"
    type="data" label="Alignments in BAM or SAM format" help="The set of
    aligned reads." /&gt; ``` Perhaps counter-intuitively, a
    ``multiple="true"`` data parameter requires at least one data input. If
    ``optional="true"`` is specified, this condition is relaxed and the
...
class Tool:
    ...
    :ivar python_template_version: This string specifies the minimum
        ...
        19.01, otherwise defaults to 3.5. Galaxy will attempt to convert
