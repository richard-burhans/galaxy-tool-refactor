...
class ChangeFormatWhen:
    ...
    :ivar input: This value must be the attribute name of the desired
        input parameter (e.g. ``input="out_format"`` above).
    :ivar value: This value must also be an attribute name of an input
        parameter (e.g. ``value="interval"`` above).
...
class Code:
    ...
    **Deprecated** do not use this unless absolutely necessary.
    ...
    This tag set provides detailed control of the way the tool is executed.
    This (optional) code can be deployed in a separate file in the same
...
class Filter:
    ...
    fragment from maf_to_interval.xml demonstrates removing items. ```xml
    &lt;param name="species" type="select" label="Select additional
    species" display="checkboxes" multiple="true" help="The species
    matching the dbkey of the alignment is always included. A separate
    history item will be created for each species."&gt; &lt;options&gt;
    &lt;filter type="data_meta" ref="input1" key="species" /&gt; &lt;filter
    type="remove_value" meta_ref="input1" key="dbkey" /&gt;
    &lt;/options&gt; &lt;/param&gt; ``` This example taken from
    ...
    :ivar key: When ``type`` is ``data_meta``, ``param_value``, or
        ``remove_value`` - this is the name of the metadata key of ref
        to filter by.
...
class Parallelism:
    ...
    :ivar split_inputs: Documentation for split_inputs
...
class Param(InputType):
    ...
    $attribute_list:name,type,optional,label,help,argument:4 ### Parameter
    Types #### ``text`` When ``type="text"``, the parameter is free form
    text and appears as a text box in the tool form. ##### Examples
    Sometimes you need labels for data or graph axes, chart titles, etc.
    This can be done using a text field. The following will create a text
    box 30 characters wide with the default value of "V1". ```xml &lt;param
    name="xlab" size="30" type="text" value="V1" label="Label for x
    axis"/&gt; ``` The ``size`` parameter can be two dimensional, if it is
    the textbox will be rendered on the tool form as a text area instead of
    a single line text box. ```xml &lt;param name="foo" type="text"
    area="True" size="5x25" /&gt; ``` $attribute_list:value,size,area:5
    #### ``integer`` and ``float`` These parameters represent whole number
    and real numbers, respectively. ##### Example ``` &lt;param
