...
@dataclass(kw_only=True)
class AssertHasJsonPropertyWithText:
    """
    Asserts the JSON document contains a property or key with the specified
    text (i.e. string) value. ```xml &lt;has_json_property_with_text
    property="color" text="red" /&gt; ``` $attribute_list::5.

    :ivar property: JSON property to search the target for.
    :ivar text: Text value to search for.
    """
...
@dataclass(kw_only=True)
class AssertHasJsonPropertyWithValue:
    """
    Asserts the JSON document contains a property or key with the specified
    JSON value. ```xml &lt;has_json_property_with_value
    property="skipped_columns" value="[1, 3, 5]" /&gt; ```
    $attribute_list::5.

    :ivar property: JSON property to search the target for.
    :ivar value: JSON-ified value to search for. This will be converted
        from an XML string to JSON with Python's json.loads function.
    """
...
@dataclass(kw_only=True)
class OutputCollectionDataElement:
    """
    :ivar data:
    ...
    data: list[OutputCollectionDataElement] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
...
class Xref:
    ...
    :ivar value:
    :ivar type_value: Type of reference - currently ``bio.tools``,
...
class AssertHasArchiveMember:
    ...
    :ivar has_n_lines:
    :ivar has_n_columns:
    :ivar has_json_property_with_value:
    :ivar has_json_property_with_text:
    :ivar is_valid_xml:
    :ivar xml_element:
...
class Param(InputType):
    ...
    ``drill_down`` Allows to select values from a hierarchy. The default
    (``hierarchy="exact"``) is that only exactly the selected options are
    used. With ``hierarchy="recurse"`` all leaf nodes in the subtree are
    used. See
    [drill_down.xml](https://github.com/galaxyproject/galaxy/blob/dev/test/functional/tools/drill_down.xml)
