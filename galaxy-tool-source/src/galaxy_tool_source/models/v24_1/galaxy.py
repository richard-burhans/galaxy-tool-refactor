...
@dataclass(kw_only=True)
class AssertHasImageCenterOfMass:
    """
    Asserts the output is an image and has a specific center of mass, or
    has an Euclidean distance of ``eps`` or less to that point (e.g.,
...
@dataclass(kw_only=True)
class AssertHasImageMeanObjectSize:
    """
    Asserts the output is an image with labeled objects which have the
    specified mean size (number of pixels), plus/minus ``eps`` (e.g.,
    ``&lt;has_image_mean_object_size mean_object_size="111.87"
    exclude_labels="0" /&gt;``).

    The labels must be unique. $attribute_list::5.
    ...
    :ivar max: An upper bound of the required mean size of the uniquely
        labeled objects.
    :ivar channel: Restricts the assertion to a specific channel of the
        image (where ``0`` corresponds to the first image channel). Must
        be used with multi-channel imags.
...
@dataclass(kw_only=True)
class AssertHasImageHeight:
    """
    Asserts the output is an image and has a specific height (in pixels),
    plus/minus ``delta`` (e.g., ``&lt;has_image_height height="512"
    delta="2" /&gt;``).

    Alternatively the range of the expected height can be specified by
...
@dataclass(kw_only=True)
class AssertHasImageNlabels:
    """
    Asserts the output is an image and has the specified number of labels,
    or unique values (e.g., ``&lt;has_image_n_labels n="187"
    exclude_labels="0" /&gt;``).

    The primary usage of this assertion is to verify the number of objects
    in images with uniquely labeled objects. $attribute_list::5.
...
class AssertHasSize:
    ...
    :ivar size: Desired size of the output (in bytes), can be suffixed
        by ``(k|M|G|T|P|E)i?``
    :ivar value: An outdated alias for `size`
...
class Command:
    ...
    * Paths should be quoted so that the Galaxy database files may contain
    ...
    ``$__user_email__`` | Email address of user ``$__app__`` | The
...
class AssertHasArchiveMember:
    ...
    :ivar has_image_width:
    :ivar has_image_height:
    :ivar has_image_channels:
    :ivar has_image_mean_intensity:
    :ivar has_image_center_of_mass:
    :ivar has_image_n_labels:
    :ivar has_image_mean_object_size:
    ...
    has_image_width: list[AssertHasImageWidth] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_image_height: list[AssertHasImageHeight] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_image_channels: list[AssertHasImageChannels] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_image_mean_intensity: list[AssertHasImageMeanIntensity] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_image_center_of_mass: list[AssertHasImageCenterOfMass] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_image_n_labels: list[AssertHasImageNlabels] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_image_mean_object_size: list[AssertHasImageMeanObjectSize] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
...
class Tool:
    ...
    leading and trailing whitespaces in `from_work_dir` attribute. - Do not
    use Galaxy Python virtual environment for `data_source` tools.
    `data_source` tools should explicitly use the `galaxy-util` package.
    ### 23.0 - Text parameters that are inferred to be optional (i.e the
    `optional` tag is not set, but the tool parameter accepts an empty
    string) are set to `None` for templating in Cheetah. Older tools
    receive the empty string `""` as the templated value. ### 24.0 - Do not
    use Galaxy python environment for `data_source_async` tools. - Drop
    request parameters received by data source tools that are not declared
    in `&lt;request_param_translation&gt;` section. ### Examples A normal
    tool: ```xml &lt;tool id="seqtk_seq" name="Convert FASTQ to FASTA"
