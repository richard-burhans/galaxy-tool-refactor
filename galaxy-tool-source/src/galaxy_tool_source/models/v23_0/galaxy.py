...
class Column:
    ...
    &lt;/options&gt; ``` The
    [gff_filter_by_feature_count](tools/filters/gff/gff_filter_by_feature_count.xml)
...
class TestOutputCompareType(Enum):
    ...
    ``re_match_multiline``, and ``contains``. In addition there is
    ``sim_size`` which is discouraged in favour of a ``has_size``
    assertion.
    """
...
class Filter:
    ...
    specified with ``ref``, or the metadata ``key`` of another input
    ``meta_ref``. * ``unique_value``: remove options that have duplicate
    entries in the given ``column``. * ``sort_by``: sort options by the
    entries of a given ``column``. If ``reverse_sort_order`` is set to
    ``true``, reverse sort order from ascending to descending. *
    ...
    columns is increased. ### Examples The following example from Mothur's
...
class TestOutput:
    ...
    compariopm if ``decompress`` is set to ``true``. BAM files are
...
class Tool:
    ...
    leading and trailing whitespaces in `from_work_dir` attribute. ### 23.0
    - Text parameters that are inferred to be optional (i.e the `optional`
    tag is not set, but the tool parameter accepts an empty string) are set
    to `None` for templating in Cheetah. Older tools receive the empty
    string `""` as the templated value. ### Examples A normal tool: ```xml
    &lt;tool id="seqtk_seq" name="Convert FASTQ to FASTA" version="1.0.0"
