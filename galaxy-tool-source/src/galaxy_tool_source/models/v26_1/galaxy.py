...
class OutputFilter:
    ...
    referred to without the ``$`` used for the Cheetah template (used in
    the ``&lt;command&gt;`` tag). Variables that are part of conditionals
    are accessed using a dictionary named after the conditional. Boolean
...
class ResourceType(Enum):
    ...
    :cvar SHM_SIZE: Size of /dev/shm. The format is
        ...
        the value is `64m`.
    :cvar TIMELIMIT: Maximum time in seconds the tool is allowed to run.
        Job will be terminated if exceeded.
    """
...
class TestCredentialsSecret:
    """
    :ivar name: Name of the credential secret (must match a secret
        defined in the tool's requirements/credentials)
    :ivar value: Test value for this secret (use public test credentials
        only)
    """
...
@dataclass(kw_only=True)
class TestCredentialsVariable:
    """
    :ivar name: Name of the credential variable (must match a variable
        defined in the tool's requirements/credentials)
    :ivar value: Test value for this variable
    """
...
class TestCredentials:
    ...
    This allows tool tests to provide credential values (variables and
    secrets) that will be injected as environment variables during test
    execution. This is intended for use with public test
    accounts/credentials only. ### Example ```xml &lt;test&gt;
    &lt;credentials name="ega_credentials"&gt; &lt;variable
    name="ega_username" value="test_user" /&gt; &lt;secret
    name="ega_password" value="test_password" /&gt; &lt;/credentials&gt;
    &lt;param name="input" value="test.txt" /&gt; &lt;output name="output"
    file="expected.txt" /&gt; &lt;/test&gt; ```.

    :ivar variable:
    :ivar secret:
    :ivar name: Name of the credentials group (must match the name
        defined in tool's requirements/credentials)
    :ivar version: Version of the credentials definition (must match the
        version defined in tool's requirements/credentials). Defaults to
        "1.0".
    """

    variable: list[TestCredentialsVariable] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    secret: list[TestCredentialsSecret] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
...
class TestAssertionsElementText:
    ...
    :ivar has_text_matching: Asserts the specified output contains text
        ...
        expression. If n is given the assertion checks for exactly n
        (nonoverlapping) occurrences. $attribute_list::5
    ...
    :ivar has_n_columns: Asserts tabular output  contains the specified
        ...
        Alternatively the range of expected occurrences can be specified
    ...
    :ivar has_n_elements_with_path: Asserts the XML output contains the
        ...
        expected number of occurrences. With ``negate`` the result of
        the assertion can be inverted. $attribute_list::5
    ...
    :ivar xml_element: Assert if the XML file contains element(s) or
        ...
        ``true`` then the sub assertions are checked for all
        occurrences. Note that all other XML assertions can be expressed
        by this assertion (Galaxy also implements the other assertions
        by calling this one). $attribute_list::5
...
class TestSection:
    ...
    credentials: list[TestCredentials] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
