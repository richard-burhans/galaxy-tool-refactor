...
class AssertHasH5Attribute:
    ...
    (``key``), e.g. ```xml &lt;has_h5_attribute key="nchroms" value="15"
    /&gt; ``` $attribute_list::5.
...
class AssertHasH5Keys:
    ...
    comma-separated list, e.g. ```xml &lt;has_h5_keys
...
@dataclass(kw_only=True)
class AssertNotHasText:
    """
    Asserts the specified ``text`` does not appear in the output (e.g.
    ``&lt;not_has_text text="chr8" /&gt;``). $attribute_list::5.

    :ivar text: Text to check for
    """
...
class ConfigInputs:
    ...
    for an example). To include a dictionary with staging paths, paths and
    metadata files set the ``data_style`` attribute to
    ``staging_path_and_source_path``. An example tool that uses
    ``staging_path_and_source_path`` is
    ...
    For tools with profile &gt;= 20.05 a select with ``multiple="true"`` is
    rendered as an array which is empty if nothing is selected. For older
    profile versions select lists are rendered as comma separated strings
    or a literal ``null`` in case nothing is selected.
    ...
    :ivar data_style: Set to 'paths' to include dataset paths in the
        ...
        a staging path, a source path and all metadata files.
    """
...
class TestOutputCompareType(Enum):
    ...
    ``re_match_multiline``, and ``contains``. In addtion there is
    ``sim_size`` which is discouraged in fafour of a ``has_size``
    assertion.
    """
...
class ActionsConditionalFilter:
    ...
    iterate: None | PermissiveBoolean = field(
        default=None,
        metadata={
            "type": "Attribute",
        },
...
@dataclass(kw_only=True)
class AssertAttributeIs:
    """
    Asserts the XML ``attribute`` for the element (or tag) with the
    specified XPath-like ``path`` is the specified ``text``, e.g. ```xml
    &lt;attribute_is path="outerElement/innerElement1" attribute="foo"
    text="bar" /&gt; ``` The assertion implicitly also asserts that an
    element matching ``path`` exists.

    With ``negate`` the result of the assertion (on the equality) can be
    inverted (the implicit assertion on the existence of the path is not
    affected). $attribute_list::5.

    :ivar path: Path to check for. Valid paths are the simplified
        subsets of XPath implemented by lxml.etree;
        https://lxml.de/xpathxslt.html for more information.
    :ivar text: Text to check for.
    :ivar negate: Negate the outcome of the assertion.
    """
    ...
    negate: PermissiveBoolean = field(
...
@dataclass(kw_only=True)
class AssertAttributeMatches:
    """
    Asserts the XML ``attribute`` for the element (or tag) with the
    specified XPath-like ``path`` matches the regular expression specified
    by ``expression``, e.g. ```xml &lt;attribute_matches
    path="outerElement/innerElement2" attribute="foo2" expression="bar\\d+"
    /&gt; ``` The assertion implicitly also asserts that an element
    matching ``path`` exists.

    With ``negate`` the result of the assertion (on the matching) can be
    inverted (the implicit assertion on the existence of the path is not
    affected). $attribute_list::5.
    ...
    :ivar path: Path to check for. Valid paths are the simplified
        subsets of XPath implemented by lxml.etree;
        https://lxml.de/xpathxslt.html for more information.
    :ivar expression: The regular expression to use.
    :ivar negate: Negate the outcome of the assertion.
    """
...
@dataclass(kw_only=True)
class AssertElementTextIs:
    """
    Asserts the text of the XML element with the specified XPath-like
    ``path`` is the specified ``text``, e.g. ```xml &lt;element_text_is
    path="BlastOutput_program" text="blastp" /&gt; ``` The assertion
    implicitly also asserts that an element matching ``path`` exists.
...
@dataclass(kw_only=True)
class AssertElementTextMatches:
    """
    Asserts the text of the XML element with the specified XPath-like
    ``path`` matches the regular expression defined by ``expression``, e.g.
    ```xml &lt;element_text_matches path="BlastOutput_version"
    expression="BLASTP\\s+2\\.2.*"/&gt; ``` The assertion implicitly also
...
@dataclass(kw_only=True)
class AssertHasElementWithPath:
    """
    ...
    path="BlastOutput_param/Parameters/Parameters_matrix" /&gt; ``` With
    ``negate`` the result of the assertion can be inverted.
    $attribute_list::5.
...
class AssertHasLine:
    ...
    If the ``line`` is expected to occur a particular number of times, this
    value can be specified using ``n``. Optionally also with a certain
    ``delta``. Alternatively the range of expected occurences can be
    specified by ``min`` and/or ``max``. $attribute_list::5.

    :ivar line: The line to check for
...
@dataclass(kw_only=True)
class AssertHasLineMatching:
    """
    Asserts a line matching the specified regular expression
    (``expression``) appears in the output (e.g. ``&lt;has_line_matching
    expression=".*\\s+127489808\\s+127494553" /&gt;``).

    If a particular number of matching lines is expected, this value can be
    specified using ``n``. Optionally also with ``delta``. Alternatively
    the range of expected occurences can be specified by ``min`` and/or
    ``max``. $attribute_list::5.

    :ivar expression: Regular expression to check for
    :ivar n: Desired number, can be suffixed by ``(k|M|G|T|P|E)i?``
    :ivar delta: Allowed difference with respect to n (default: 0), can
        be suffixed by ``(k|M|G|T|P|E)i?``
    :ivar min: Minimum number (default: -infinity), can be suffixed by
...
@dataclass(kw_only=True)
class AssertHasNcolumns:
    """
    Asserts tabular output (actually only the first line) contains the
    specified number (``n``) of columns (e.g. ``&lt;has_n_columns
    n="3"/&gt;``) optionally also with ``delta``.

    Alternatively the range of expected occurences can be specified by
    ``min`` and/or ``max``. Optionally a column separator (``sep``, default
    is ``\\t``) `and comment character(s) can be specified (``comment``,
    default is empty string), then the first non-comment line is used for
    determining the number of columns. $attribute_list::5.
...
@dataclass(kw_only=True)
class AssertHasNelementsWithPath:
    """
    Asserts the XML output contains the specified number (``n``, optionally
    with ``delta``) of elements (or tags) with the specified XPath-like
    ``path``, e.g. ```xml &lt;has_n_elements_with_path n="9"
    path="BlastOutput_iterations/Iteration/Iteration_hits/Hit/Hit_num"
    /&gt; ``` Alternatively to ``n`` and ``delta`` also the ``min`` and
    ``max`` attributes can be used to specify the range of the expected
...
@dataclass(kw_only=True)
class AssertHasNlines:
    """
    Asserts that an output contains ``n`` lines, allowing for a difference
    of ``delta`` (default is 0), e.g. ``&lt;has_n_lines n="3"
    delta="1"/&gt;``.

    Alternatively the range of expected occurences can be specified by
    ``min`` and/or ``max``. $attribute_list::5.
...
class AssertHasSize:
    """
    Asserts the output has a specific size (in bytes) of ``value`` plus
    minus ``delta``, e.g. ``&lt;has_size value="10000" delta="100" /&gt;``.

    Alternatively the range of the expected size can be specified by
    ``min`` and/or ``max``. $attribute_list::5.

    :ivar value: Desired size of the output (in bytes), can be suffixed
        by ``(k|M|G|T|P|E)i?``
    :ivar delta: Maximum allowed size difference (default is 0). The
        observed size has to be in the range ``value +- delta``. Can be
        suffixed by ``(k|M|G|T|P|E)i?``
    :ivar min: Minimum expected size, can be suffixed by
        ``(k|M|G|T|P|E)i?``
    :ivar max: Maximum expected size, can be suffixed by
        ``(k|M|G|T|P|E)i?``
    :ivar negate: Negate the outcome of the assertion.
    """
...
class AssertHasText:
    """
    Asserts the specified ``text`` appears in the output (e.g.
    ``&lt;has_text text="chr7"&gt;``).

    If the ``text`` is expected to occur a particular number of times, this
    value can be specified using ``n``. Optionally also with a certain
    ``delta``. Alternatively the range of expected occurences can be
    specified by ``min`` and/or ``max``. $attribute_list::5.
...
@dataclass(kw_only=True)
class AssertHasTextMatching:
    """
    Asserts text matching the specified regular expression (``expression``)
    appears in the output (e.g. ``&lt;has_text_matching
    expression="1274\\d+53" /&gt;`` ).

    If the regular expression is expected to match a particular number of
    times, this value can be specified using ``n``. Note only
    non-overlapping occurences are counted. Optionally also with a certain
    ``delta``. Alternatively the range of expected occurences can be
    specified by ``min`` and/or ``max``. $attribute_list::5.
...
@dataclass(kw_only=True)
class AssertXmlelement:
    """
    Assert if the XML file contains element(s) or tag(s) with the specified
    [XPath-like ``path``](https://lxml.de/xpathxslt.html).

    If ``n`` and ``delta`` or ``min`` and ``max`` are given also the number
    of occurences is checked. ```xml &lt;assert_contents&gt;
    &lt;xml_element path="./elem"/&gt; &lt;xml_element
    path="./elem/more[2]"/&gt; &lt;xml_element path=".//more" n="3"
    delta="1"/&gt; &lt;/assert_contents&gt; ``` With ``negate="true"`` the
    outcome of the assertions wrt the precence and number of ``path`` can
    be negated. If there are any sub assertions then check them against -
    the content of the attribute ``attribute`` - the element's text if no
    attribute is given ```xml &lt;assert_contents&gt; &lt;xml_element
    path="./elem/more[2]" attribute="name"&gt; &lt;has_text_matching
    expression="foo$"/&gt; &lt;/xml_element&gt; &lt;/assert_contents&gt;
    ``` Sub-assertions are not subject to the ``negate`` attribute of
    ``xml_element``. If ``all`` is ``true`` then the sub assertions are
    checked for all occurences. Note that all other XML assertions can be
    expressed by this assertion (Galaxy also implements the other
    assertions by calling this one). $attribute_list::5.
    ...
    :ivar attribute: The name of the attribute to apply sub-assertion
        on. If not given then the element text is used
    :ivar n: Desired number, can be suffixed by ``(k|M|G|T|P|E)i?``
    :ivar delta: Allowed difference with respect to n (default: 0), can
        be suffixed by ``(k|M|G|T|P|E)i?``
    :ivar min: Minimum number (default: -infinity), can be suffixed by
...
class Command:
    ...
    * Paths should be quoted so that the Galaxy database files may contain
    ...
    converter](https://github.com/galaxyproject/galaxy/blob/master/lib/galaxy/datatypes/converters/biom.xml).
...
class Validator:
    ...
    data set is in state OK. - ``dataset_metadata_in_range``: Check if a
    numeric metadata value is within a given range. -
    ...
    **matches** the value, i.e. appears at the beginning of the value. To
    ...
    :ivar type_value: Valid values are: ``expression``, ``regex``,
        ``in_range``, ``length``, ``metadata``, ``unspecified_build``,
        ``no_options``, ``empty_field``,
...
class Xref:
    ...
    :ivar value:
    :ivar type_value: Type of reference - currently ``bio.tools`` and
        ``bioconductor`` are the only supported options.
    """
...
@dataclass(kw_only=True)
class AssertElementText:
    """
    This tag allows the developer to recurisively specify additional
    assertions as child elements about just the text contained in the
    element specified by the XPath-like ``path``, e.g. ```xml
    &lt;element_text
    path="BlastOutput_iterations/Iteration/Iteration_hits/Hit/Hit_def"&gt;
    &lt;not_has_text text="EDK72998.1" /&gt; &lt;/element_text&gt; ``` The
    assertion implicitly also asserts that an element matching ``path``
    exists.

    With ``negate`` the result of the implicit assertions can be inverted.
    The sub-assertions, which have their own ``negate`` attribute, are not
    affected by ``negate``. $attribute_list::5.

    :ivar has_text:
    :ivar not_has_text:
    :ivar has_text_matching:
    :ivar has_line:
    :ivar has_line_matching:
    :ivar has_n_lines:
    :ivar path: Path to check for. Valid paths are the simplified
        subsets of XPath implemented by lxml.etree;
        https://lxml.de/xpathxslt.html for more information.
    ...
    has_text: list[AssertHasText] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    not_has_text: list[AssertNotHasText] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_text_matching: list[AssertHasTextMatching] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_line: list[AssertHasLine] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_line_matching: list[AssertHasLineMatching] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_n_lines: list[AssertHasNlines] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
...
class ParamOptions:
    ...
    dynamically generated lists of options. For data parameters this tag
    can be used to restrict possible input datasets to datasets that match
    the ``dbkey`` of another data input by including a ``data_meta``
    filter. See for instance here:
    [/tools/maf/interval2maf.xml](https://github.com/galaxyproject/galaxy/blob/master/tools/maf/interval2maf.xml)
    ...
    locally or a dataset selected from the current history. There are at
    least five basic ways to use this tag - four of these correspond to a
    ``from_XXX`` attribute on the ``options`` directive and the other is to
    exclusively use ``filter``s to populate options. * ``from_data_table``
    - The options for the select list are dynamically obtained from a file
    specified in the Galaxy configuration file ``tool_data_table_conf.xml``
    or from a Tool Shed installed data manager. * ``from_dataset`` - The
    options for the select list are dynamically obtained from input dataset
    selected for the tool from the current history. * ``from_file`` - The
    options for the select list are dynamically obtained from a file. This
    mechanism is discouraged in favor of the more generic
    ``from_data_table``. * ``from_parameter`` - The options for the select
    list are dynamically obtained from a parameter. * Using ``filter``s -
    various filters can be used to populate options, see examples in the
...
class Requirements:
    ...
    ``container`` tags described in greater detail below. ``requirement``s
    describe software packages and other individual computing requirements
    required to execute a tool, while ``container``s describe Docker or
    Singularity containers that should be able to serve as complete
    descriptions of the runtime of a tool.
    """
...
class Xrefs:
    ...
    tool is dada2 --&gt; &lt;xrefs&gt; &lt;xref
    type="bio.tools"&gt;dada2&lt;/xref&gt; &lt;xref
    type="bioconductor"&gt;dada2&lt;/xref&gt; &lt;/xrefs&gt; &lt;!--
...
class AssertHasArchiveMember:
    ...
    The path is a regular expression that is matched against the full paths
    of the objects in the compressed file (remember that "matching" means
    it is checked if a prefix of the full path of an archive member is
    described by the regular expression). Valid archive formats include
    ``.zip``, ``.tar``, and ``.tar.gz``. Note that depending on the archive
    creation method: - full paths of the members may be prefixed with
    ``./`` - directories may be treated as empty files ```xml
    &lt;has_archive_member path="./path/to/my-file.txt"/&gt; ``` With ``n``
    and ``delta`` (or ``min`` and ``max``) assertions on the number of
    archive members matching ``path`` can be expressed. The following could
    be used, e.g., to assert an archive containing n&amp;plusmn;1 elements
    out of which at least 4 need to have a ``txt`` extension. ```xml
    &lt;has_archive_member path=".*" n="10" delta="1"/&gt;
    &lt;has_archive_member path=".*\\.txt" min="4"/&gt; ``` In addition the
    tag can contain additional assertions as child elements about the first
    member in the archive matching the regular expression ``path``. For
    instance ```xml &lt;has_archive_member path=".*/my-file.txt"&gt;
    &lt;not_has_text text="EDK72998.1"/&gt; &lt;/has_archive_member&gt; ```
    If the ``all`` attribute is set to ``true`` then all archive members
    are subject to the assertions. Note that, archive members matching the
    ``path`` are sorted alphabetically. The ``negate`` attribute of the
    ``has_archive_member`` assertion only affects the asserts on the
    presence and number of matching archive members, but not any
    sub-assertions (which can offer the ``negate`` attribute on their own).
    The check if the file is an archive at all, which is also done by the
    function, is not affected. $attribute_list::5.
    ...
    :ivar is_valid_xml:
    :ivar xml_element:
    :ivar has_element_with_path:
    :ivar has_n_elements_with_path:
    :ivar element_text_matches:
    :ivar element_text_is:
    :ivar attribute_matches:
    :ivar attribute_is:
    :ivar element_text:
    :ivar has_h5_keys:
    ...
    has_size: list[AssertHasSize] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    ...
    has_n_columns: list[AssertHasNcolumns] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    is_valid_xml: list[AssertIsValidXml] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    xml_element: list[AssertXmlelement] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    has_element_with_path: list[AssertHasElementWithPath] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    ...
    element_text_matches: list[AssertElementTextMatches] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    element_text_is: list[AssertElementTextIs] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    attribute_matches: list[AssertAttributeMatches] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    attribute_is: list[AssertAttributeIs] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
    )
    element_text: list[AssertElementText] = field(
        default_factory=list,
        metadata={
            "type": "Element",
        },
...
class Param(InputType):
    ...
    [drill_down.xml](https://github.com/galaxyproject/galaxy/blob/dev/test/functional/tools/drill_down.xml)
    $attribute_list:hierarchy:5 #### ``data_collection`` The following will
    create a parameter that only accepts paired FASTQ files grouped into a
    collection. ##### Examples ```xml &lt;param name="inputs"
    type="data_collection" collection_type="paired" label="Input FASTQs"
    format="fastq"&gt; &lt;/param&gt; ``` More detailed information on
    writing tools that consume collections can be found in the [planemo
    ...
    collection_type: None | str = field(
        ...
        metadata={
            "type": "Attribute",
            "pattern": r"(list|paired)([:,](list|paired))*",
        },
    )
...
class Test:
    ...
    expect_test_failure: PermissiveBoolean = field(
        default=PermissiveBoolean.FALSE,
        metadata={
            "type": "Attribute",
        },
...
class Tool:
    ...
    Assume sort order for collection elements. ### 21.09 - Do not strip
    leading and trailing whitespaces in `from_work_dir` attribute. ###
    Examples A normal tool: ```xml &lt;tool id="seqtk_seq" name="Convert
    FASTQ to FASTA" version="1.0.0" profile="16.04" &gt; ``` A
    ``data_source`` tool contains a few more relevant attributes. ```xml
    &lt;tool id="ucsc_table_direct1" name="UCSC Main" version="1.0.0"
    hidden="false" profile="16.01" tool_type="data_source"
    URL_method="post"&gt; ```.
    ...
    :ivar license: This string specifies any full URI or a a short
        ...
        this tool wrapper. The tool wrapper version can be indepedent of
        the underlying tool. This license covers the tool XML and
