...
``uv sync`` populates ``src/galaxy_tool_source/models/`` for development. The codegen
itself lives in ``galaxy_tool_source._codegen`` (which never imports hatchling);
...
class CustomBuildHook(BuildHookInterface):
    ...
    def clean(self, versions: list[str]) -> None:
        ...
        self._ensure_src_on_path()
        from galaxy_tool_source._codegen import clean_generated
