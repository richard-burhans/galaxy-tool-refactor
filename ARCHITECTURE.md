...
| 3.5 | **advisory checks** | `galaxy-tool-lint` | Detect-only IUC best-practice + planemo-parity checks (72; `CheckRule`, `detect_violations`). Read-only LBYL queries. Depends only on tiers 1 + 0.5. |
...
- **The behavior gate** (`behavior_gate.py`, codemod §45): the behavioural cap on
  the opt-in `modernize` upgrade walk: `blocking_codes` (applicable, non-auto-fixed
  `must_fix` codes over `(baseline, latest]`, with auto-fixability proven by
  executing the mapped `RuntimeGatedFix` on a copy and re-detecting) and
  `behavior_ceiling` (the newest vendored profile below the first blocker). Pure
...
- **`lint-skip`** — opt-in convenience: prune planemo `.lint_skip` suppressions the
  ...
  and clean on every tool in the directory). Writes the tool XML and its sidecar, so
  never part of `format` / `upgrade`. Wraps `facade.reconcile_lint_skip` (cli §D19,
  registry D24).
