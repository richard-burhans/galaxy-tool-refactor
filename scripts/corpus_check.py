...
``rules`` — sweep the corpus through **every** GTR rule in isolation (one rule
at a time, no others) and write ``docs/corpus_rule_stats.md``:

* fmt rules (GTR001/003/004) — idempotence + no-crash only (a rule run without
  its usual predecessors emits valid-but-non-canonical output, which is fine);
* codemods (GTR002, GTR005–GTR012) — idempotence + post-codemod validity, with
...
def _gtx_reference_table(intro: str, /) -> list[str]:
    """Render the cross-tier GTR rule glossary (fmt + codemod) under *intro*.

    fmt-tier rules (``all_rules()``) and codemod-tier rules
    (``coded_codemods()``) share the ``RuleMeta`` vocabulary, so the glossary is
    generated from live metadata rather than hand-maintained. The advisory
    checks (tier 3.5) carry their own codes and are reported separately on the
    ``check`` sweep page, so they are not part of this GTR glossary.
    """
...
def _fmt_format_rule_reference_table() -> list[str]:
    """The GTR glossary for the fmt stat page (codemod rows are reference-only)."""
    return _gtx_reference_table(
        "What each GTR rule does, across both tiers. *fmt*-tier rules are the "
...
# (a rule like GTR003 run without GTR001 emits valid-but-non-canonical output —
...
def _rule_format_reference_table() -> list[str]:
    """The GTR glossary for the per-rule isolation stat page."""
    return _gtx_reference_table(
        "What each GTR rule does, across both tiers. The isolation tables below "
    ...
    *,
    ...
    lines: list[str] = [
        ...
        (
            ...
            f"{date.today().isoformat()}. Each GTR rule is run **in isolation** "
...
def _rules_main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Run each GTR rule (fmt + codemod) in isolation across the corpus, "
...
# phases the CLI runs. Covers the detect-only checks. Writes
...
def _check_rule_registry() -> dict[str, _CheckRuleStat]:
    ...
    codemods + advisory), keyed by code."""
...
def _check_detect(document: ToolDocument) -> list[Violation]:
    """Unified detect over one tool — canonical codemods + fmt + advisory.
...
def _check_stats_lines(*, source: str, state: _CheckSweepState) -> list[str]:
    ...
    lines: list[str] = [
        ...
        (
            ...
            f"codemods + cosmetic fmt rules (*fixable*) plus the advisory "
    ...
    lines.extend(
        _check_format_table(
            state,
            detect_only=False,
            heading="## Fixable rules (GTR — what `format` would change)",
            blurb=(
    ...
    lines.extend(
        _check_format_table(
            ...
            blurb=(
                ...
                "them but does not fail unless `--strict`. The `.2` rows are the "
                "advisory residual of a partition practice (the fixable `.1` "
                "sibling is in the codemod table)."
            ),
        )
    )
    lines.append("")
...
def _check_main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "+ advisory checks) and report per-rule violation counts, "
...
from galaxy_tool_refactor_registry.resolve import resolve_upgrade_codes  # noqa: E402

_UPGRADE_REGRESSIONS = (
    _REPO_ROOT / "galaxy-tool-refactor-registry" / "tests" / "data" / "regressions"
)
_UPGRADE_ERRORS_FILE = _CORPUS_DATA_DIR / "upgrade_gate_errors.json"


@dataclass
class _UpgradeGateOutcome:
    """Classification of one tool's pass through the gated upgrade."""
...
def _upgrade_gate_exercise(path: Path) -> _UpgradeGateOutcome:
    ...
    try:
        ...
        blockers = (
            behavior_gate.blocking_codes(original, baseline=baseline)
        ...
        after_placeable = (
            effective_after is not None
            and behavior_gate.placeable_baseline(effective_after)
        )
        if (
            blockers
            and after_placeable
            and Version(str(effective_after))
            >= min(Version(change.profile) for change in blockers)
        ):
            return _UpgradeGateOutcome(
                "gate-cap-violated",
                f"gate-cap-violated:{blockers[0].code}",
                f"declared {effective_after} despite a blocker at "
                f"{blockers[0].profile} ({blockers[0].code})",
            )
        if placeable and baseline is not None and after_placeable:
            crossed_unfixed = [
                change
                for change in upgrade_codes_applicable(
                    document=original,
                    from_profile=baseline,
                    to_profile=str(effective_after),
                )
                if change.level == "must_fix"
                and change.code not in result.auto_fixed_codes
            ]
            if crossed_unfixed:
                return _UpgradeGateOutcome(
                    "must-fix-crossed",
                    f"must-fix-crossed:{crossed_unfixed[0].code}",
                    ", ".join(change.code for change in crossed_unfixed),
                )
...
def _upgrade_main(argv: list[str]) -> int:
    ...
    if "toolshed" in sources and not _TOOLSHED_ROOT.exists():
        ...
        return 1
    full_sweep = not (args.limit or args.repo)
    if full_sweep and _toolshed_corpus_incomplete(sources):
        return 1
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "subcommand",
        choices=("validate", "fmt", "codemod", "rules", "check", "upgrade"),
        help=(
    ...
    if args.subcommand == "upgrade":
        return _upgrade_main(remaining)
