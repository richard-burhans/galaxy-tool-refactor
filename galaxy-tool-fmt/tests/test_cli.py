"""Tests for the ``galaxy-tool-fmt`` CLI."""
...
from galaxy_tool_fmt.cli import main
from galaxy_tool_fmt.cli_support import is_macros_root, is_tool_root, make_backup
...
def test_cli_does_not_reorder_attributes(tmp_path: Path) -> None:
    ...
    left in that order by ``galaxy-tool-fmt``.
    """
    param_out_of_order = (
        b"<tool id='t' name='T' version='0.1'>"
