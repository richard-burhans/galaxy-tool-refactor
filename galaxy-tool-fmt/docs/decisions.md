...
This file records architectural decisions for `galaxy-tool-fmt`,
...
version in their lockfile (`galaxy-tool-fmt==x.y.z`). The formatter
...
Single package (`galaxy-tool-fmt`) for now. A shared rule-engine
...
> `galaxy-tool-codemod` as `ReorderParamAttributes` (a structural
...
> `galaxy-tool-codemod` (the helper to
...
`src/galaxy_tool_fmt/attribute_ordering.py`:
...
this package and re-implemented in `galaxy-tool-codemod` as the
...
- **`format_tool_document` is now cosmetic-only.** It no longer imports
  `galaxy-tool-codemod`, runs only the three remaining cosmetic
  rules (GTR001 indent, GTR003 blank line, GTR004 empty-element
  shorthand), and works with just `galaxy-tool-xml + galaxy-tool-fmt`
  installed.
- **`galaxy-tool-codemod` is an optional `[canonical]` extra** of
  this package, not a hard dependency. Declared as
  `[project.optional-dependencies] canonical = ["galaxy-tool-codemod"]`.
- **The CLI orchestrates both layers.** `galaxy-tool-fmt`'s CLI
...
state). Or, hard-depend on `galaxy-tool-codemod` (the
...
See also `galaxy-tool-codemod/docs/decisions.md` §9 for the
...
- `test_format_tool_document_does_not_import_codemod_package`
  ...
  `galaxy_tool_codemod` modules loaded after
...
```sh
uv sync
uv run --package galaxy-tool-fmt pytest galaxy-tool-fmt/tests/test_framework.py
uv run --package galaxy-tool-fmt pytest galaxy-tool-fmt/tests/test_cli.py
```
...
- **`galaxy-tool-fmt`'s CLI is cosmetic-only again.** `cli.py` no longer
  ...
  hint about).
- **The `[canonical]` extra is removed**, along with fmt's `galaxy-tool-codemod`
...
- The two §D10 CLI tests (`…runs_canonical_codemods…`,
  `…cosmetic_only_hint…`) were removed; `galaxy-tool-fmt`'s CLI test now
...
uv run --package galaxy-tool-fmt pytest galaxy-tool-fmt/tests/test_corpus_check.py
```

## D14 (2026-05-30) — Cosmetic detect (lint) phase: `detect_tool_document` (PR2)

**Date:** 2026-05-30. Reproduced-by: `uv run --package galaxy-tool-fmt
pytest galaxy-tool-fmt/tests/test_detect.py`; corpus gate `uv run python -m
scripts.corpus_check fmt`.

PR2 of the detect/fix rule-split effort (tier-2 framework landed in PR1 — see
`galaxy-tool-codemod/docs/decisions.md` §19, `galaxy-tool-refactor-rules`
...
**Date:** 2026-05-30. Reproduced-by: `uv run --package galaxy-tool-fmt
pytest galaxy-tool-fmt/tests/test_subset.py`.
...
galaxy-tool-fmt/tests/test_subset.py galaxy-tool-fmt/tests/test_cli.py`.
...
- **`cli_support` gained an optional `macro_transform`** and an `is_macros_root`
  ...
  the **cosmetic `galaxy-tool-fmt` CLI** opts in and formats both kinds. The
...
galaxy-tool-fmt pytest galaxy-tool-fmt/tests/test_rule_empty_element.py`.
...
(`../../docs/behavior_preservation.md`). Reproduced-by: `uv run --package
galaxy-tool-fmt pytest galaxy-tool-fmt/tests/test_rule_indent.py` and
`uv run python -m scripts.corpus_check fmt`.
...
and `uv run --package galaxy-tool-fmt pytest
galaxy-tool-fmt/tests/test_rule_empty_element.py
galaxy-tool-fmt/tests/test_rule_indent.py`.
