...
- the **advisory** sub-rules (tier 3.5, codes ``GTR018.2`` / ``GTR019.2``) flag the
  *residual* — a body that needs CDATA (``needs_cdata``) but is **not**
  ``cdata_wrappable`` (mixed-content or carries a ``]]>`` terminator).
...
def cdata_wrappable(element: etree._Element, /) -> bool:
    ...
    body can't be one section), is **not already** wrapped, and contains no ``]]>``
    terminator (which cannot live inside a section). This is exactly the fix's
    eligibility; the advisory residual is ``needs_cdata and not cdata_wrappable``.
    """
    text = element.text
