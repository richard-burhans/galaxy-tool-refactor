...
Generated by `python -m scripts.corpus_check check` on 2026-06-09. Runs the exact detect phases the `galaxy-tool-refactor check` command reports — the canonical codemods + cosmetic fmt rules (*fixable*) plus the advisory best-practice checks (*advisory*, detect-only) — across the combined corpus (github + toolshed, sha256-deduplicated), and counts, per rule, how many tools carry the finding. Unlike the per-rule **isolation** page (`corpus_rule_stats.md`, which counts every emitted edit to QA each rule alone), these are genuine per-occurrence violation counts. Every parseable `<tool>` is swept (no validity gate, matching `check`).
