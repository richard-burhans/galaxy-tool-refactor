...
Generated by `python -m scripts.corpus_check check` on 2026-06-14. Runs the exact detect phases the `galaxy-tool-refactor check` command reports — the canonical codemods + cosmetic fmt rules (*fixable*) plus the advisory best-practice checks (*advisory*, detect-only) — across the combined corpus (github + toolshed, sha256-deduplicated), and counts, per rule, how many tools carry the finding. Unlike the per-rule **isolation** page (`corpus_rule_stats.md`, which counts every emitted edit to QA each rule alone), these are genuine per-occurrence violation counts. Every parseable `<tool>` is swept (no validity gate, matching `check`).

**9,304** tools swept; **9,296** have at least one finding (**8,941** a fixable one, **9,213** an advisory one); 0 crashed.
...
|---|---|---:|---:|---:|---|
| GTR001 | fmt | 7,134 | 76.7% | 339,383 | Canonical 4-space indentation; no tabs. |
| GTR002 | codemod | 6,639 | 71.4% | 37,462 | Reorder every `<param>` element's attributes to the IUC convention. |
| GTR004 | fmt | 1,584 | 17.0% | 2,790 | Collapse empty-with-whitespace leaves to `<foo/>` form. |
| GTR005 | codemod | 1,063 | 11.4% | 1,063 | Reorder the root `<tool>` element's attributes to the documented prefix. |
...
| GTR018.1 | codemod | 3,253 | 35.0% | 3,253 | Wrap a pure-text `<command>` body in CDATA (IUC #34). |
| GTR019.1 | codemod | 3,728 | 40.1% | 3,971 | Wrap a pure-text `<help>` body in CDATA (IUC #42). |
| GTR020.1 | codemod | 3,234 | 34.8% | 3,234 | Single-quote provably-single-valued Cheetah variables in `<command>` (bare single-token params, $__…__ path built-ins, space-free attrs). |
...
| GTR036 | codemod | 1 | 0.0% | 1 | Replace a deprecated `<outputs>``<output type="data">` with `<data>`, and `<output type="collection">` with `<collection>` via Galaxy's own attribute remap (expression / degenerate outputs are left for the advisory check). |
...
| GTR020.2 | check | 4,173 | 44.9% | 19,690 | Single-quote `<command>` Cheetah vars: the non-provable residual. |
| GTR021 | check | 2,671 | 28.7% | 2,671 | Tool should ship at least one functional `<test>`. |
| GTR023 | check | 1,915 | 20.6% | 1,915 | Tool id should use lowercase letters, digits, and '_.+-'. |
| GTR024 | check | 61 | 0.7% | 61 | Tool version should be PEP 440 or a @...@ version macro. |
| GTR025 | check | 5,338 | 57.4% | 5,338 | Tool should declare `<requirements>`. |
| GTR026 | check | 3,812 | 41.0% | 3,812 | Tool should declare error handling (detect_errors or `<stdio>`). |
| GTR027 | check | 8,340 | 89.6% | 8,340 | Tool should declare EDAM topics/operations or `<xrefs>`. |
...
| GTR038 | check | 6,720 | 72.2% | 6,720 | Tool should declare a non-empty `<citation>` (doi/bibtex). |
...
| GTR087 | check | 611 | 6.6% | 1,595 | A test should set expect_num_outputs when outputs are filtered. |
| GTR088 | check | 188 | 2.0% | 270 | A test should assert outputs or expectations. |
| GTR089.2 | check | 184 | 2.0% | 184 | A `<help>` body should be valid reStructuredText (the non-fixable residual). |
...
| GTR102 | check | 339 | 3.6% | 492 | A boolean param should not gate other options in `<command>` (use a `<conditional>`/select). |
