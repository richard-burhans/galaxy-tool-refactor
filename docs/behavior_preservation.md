...
| GTR020.1 | SingleQuoteCommandVars | runtime | **REFUTED → FIXED** | quoted multi-flag `select` values (PR #110); flag-idiom booleans `falsevalue=""` (2026-06-11, §44); scope **extended** to output `<data>` files (2026-06-13, §51) — sound by the data-input value-domain argument (Galaxy-assigned single-token path, no splat idiom); then **narrowed** to the IUC rule's file scope (2026-06-13, §52) — quotes input/output files only, dropping the safe-but-out-of-scope select/number/boolean/attr/built-in quoting (a no-op removal, not a behaviour change) |
