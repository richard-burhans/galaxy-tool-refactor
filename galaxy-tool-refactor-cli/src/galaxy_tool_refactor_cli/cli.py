...
CI shell). ``format`` and ``upgrade`` share fmt's file-walking /
...
from galaxy_tool_refactor_cli import gate_suggest
...
@main.command(name="gate-suggest", hidden=True)
@click.option(
...
def gate_suggest_command(
    ...
    if not result.suggestions and not result.skipped:
        ...
        return
    payload = gate_suggest.review_payload(
    ...
    if dry_run:
        click.echo(gate_suggest.dump_payload(payload))
        return
    if not repo or pr is None:
        raise click.UsageError("--repo and --pr are required unless --dry-run")
    if not gate_suggest.post_review(repo, pr, payload):
        raise SystemExit(1)
