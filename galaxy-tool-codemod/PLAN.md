...
- `pyproject.toml`, `src/galaxy_tool_codemod/`, `tests/`
...
The two structural rules previously in `galaxy-tool-fmt` have
...
### Step 1: `Module` dataclass (`src/galaxy_tool_codemod/module.py`)
...
### Step 2: `Cursor` class (`src/galaxy_tool_codemod/cursor.py`)
...
### Step 3: `parse_module` function (`src/galaxy_tool_codemod/parse.py`)
...
### Step 4: Public re-exports (`src/galaxy_tool_codemod/__init__.py`)
...
3. `ruff check galaxy-tool-codemod/src` clean.
4. `mypy --config-file galaxy-tool-codemod/pyproject.toml galaxy-tool-codemod/src` clean.
5. M1 tests green per Step 5 (six cases).
6. `parse_module` re-exported (importable) from `galaxy_tool_codemod.parse`.
...
2. `uv run pytest galaxy-tool-codemod/tests/` runs the smoke test green.
3. `uv run ruff check galaxy-tool-codemod/src` is clean.
4. `uv run mypy --config-file galaxy-tool-codemod/pyproject.toml galaxy-tool-codemod/src` reports no issues.
