"""Unified, code-addressable rule registry + presets for the refactor tiers (tier 3.6).
...
- ``galaxy_tool_refactor_registry.facade`` — ``run`` / ``upgrade`` / ``detect``
  (the library-first, structured entry points) plus ``list_presets`` /
...
- ``galaxy_tool_refactor_registry.presets`` — the named preset code sets.
